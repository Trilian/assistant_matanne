...
1. [Vue d'ensemble des phases](#phase-0--vue-densemble)
2. [Sprint A — Corrections critiques](#sprint-a--corrections-critiques-bugs--qualité)
3. [Sprint B — Tests manquants](#sprint-b--tests-manquants)
4. [Sprint C — Pages IA Avancée](#sprint-c--pages-ia-avancée)
5. [Sprint D — Inter-modules & Event Bus](#sprint-d--inter-modules--event-bus)
6. [Sprint E — Notifications enrichies](#sprint-e--notifications-enrichies)
7. [Sprint F — Admin & DX](#sprint-f--admin--dx)
8. [Sprint G — UX & Simplification flux](#sprint-g--ux--simplification-flux)
9. [Sprint H — Consolidation SQL & Organisation code](#sprint-h--consolidation-sql--organisation-code)
10. [Sprint I — Innovations long terme](#sprint-i--innovations-long-terme)
11. [Référentiel Bugs](#référentiel-bugs)
12. [Référentiel Gaps & Features manquantes](#référentiel-gaps--features-manquantes)
13. [Référentiel Consolidation SQL](#référentiel-consolidation-sql)
14. [Référentiel Couverture de tests](#référentiel-couverture-de-tests)
15. [Référentiel Documentation](#référentiel-documentation)
16. [Référentiel Interactions intra-modules](#référentiel-interactions-intra-modules)
17. [Référentiel Interactions inter-modules](#référentiel-interactions-inter-modules)
18. [Référentiel Opportunités IA](#référentiel-opportunités-ia)
19. [Référentiel Jobs CRON](#référentiel-jobs-cron)
20. [Référentiel Notifications](#référentiel-notifications)
21. [Référentiel Mode Admin](#référentiel-mode-admin)
22. [Référentiel Simplification UX](#référentiel-simplification-ux)
23. [Référentiel Organisation du code](#référentiel-organisation-du-code)
24. [Référentiel Axes d'innovation](#référentiel-axes-dinnovation)
25. [Annexes](#annexes)
...
PRIORITÉ HAUTE (fonctionnalités core)
  Sprint C ── Pages IA Avancée               → ✅ TERMINE (31 mars 2026)
...
|---|-------|-----|--------|-------|
| A.11 | Cache d'idempotence courses → Redis ou DB au lieu de mémoire | B5 | M | Multi-worker safe |
| A.12 | Invalidation cache auto quand modèles modifiés en DB | B9 | M | Trigger PostgreSQL → invalidation |
...
## Sprint B — Tests manquants ✅ IMPLÉMENTÉ (phase 1)

**Objectif** : Couverture backend 75% → 85%, initier les tests unit frontend
**Date de complétion (phase 1)** : 31 mars 2026

### Backend — Routes API sans tests (9 routes)

| # | Tâche | Route | Fichier test | Priorité | Effort | Statut |
|---|-------|-------|--------------|----------|--------|--------|
| B.1 | Tests route dashboard | `dashboard` | `tests/api/test_routes_dashboard.py` | 🔴 | M | ✅ |
| B.2 | Tests route famille_jules | `famille_jules` | `tests/api/test_routes_famille_jules.py` | 🔴 | M | ✅ |
| B.3 | Tests route famille_budget | `famille_budget` | `tests/api/test_routes_famille_budget.py` | 🟡 | M | ✅ |
| B.4 | Tests route famille_activites | `famille_activites` | `tests/api/test_routes_famille_activites.py` | 🟡 | M | ✅ |
| B.5 | Tests route maison_projets | `maison_projets` | `tests/api/test_routes_maison_projets.py` | 🟡 | M | ✅ |
| B.6 | Tests route maison_jardin | `maison_jardin` | `tests/api/test_routes_maison_jardin.py` | 🟡 | M | ✅ |
| B.7 | Tests route maison_finances | `maison_finances` | `tests/api/test_routes_maison_finances.py` | 🟡 | M | ✅ |
| B.8 | Tests route maison_entretien | `maison_entretien` | `tests/api/test_routes_maison_entretien.py` | 🟡 | M | ✅ |
| B.9 | Tests route partage | `partage` | `tests/api/test_routes_partage.py` | 🟢 | S | ✅ |
...
| # | Tâche | Type | Fichier(s) | Effort | Statut |
|---|-------|------|-----------|--------|--------|
| B.10 | Tests custom hooks (utiliser-api, utiliser-auth) | Unit (Vitest) | `frontend/src/__tests__/hooks/utiliser-api.test.ts`, `frontend/src/__tests__/hooks/utiliser-auth.test.ts` | M | ✅ |
| B.11 | Tests composants critiques (formulaire-recette, sidebar) | Unit (Vitest) | `frontend/src/__tests__/cuisine/formulaire-recette.test.tsx`, `frontend/src/__tests__/components/barre-laterale.test.tsx` | M | ✅ |
| B.12 | Tests rendering pages hub | Unit (Vitest) | `frontend/src/__tests__/pages/hubs.test.tsx` | M | ✅ |
| B.13 | E2E complets par module (pas juste smoke) | E2E (Playwright) | `frontend/e2e/cuisine-complet.spec.ts` | L | ◐ Partiel (module Cuisine) |
| B.14 | Tests d'accessibilité (axe-core) | E2E | `frontend/e2e/accessibility.spec.ts` | M | ✅ |

### CI — Validation schéma

| # | Tâche | Réf | Effort | Statut |
|---|-------|-----|--------|--------|
| B.15 | Ajouter `test_schema_coherence.py` au pipeline CI | S3 | S | ✅ (`.github/workflows/tests.yml`, `.github/workflows/backend-tests.yml`) |

---

## Sprint C — Pages IA Avancée ✅ TERMINE

**Objectif** : Finaliser les 12 pages IA avancée + clients API + documentation
...
| C.4 | `/ia-avancee/optimisation-routines` *(renomme depuis optimisation-stock)* | ✅ Page complete et connectee API | Cloture | M |
| C.5 | `/ia-avancee/analyse-photo` *(renomme depuis analyse-habitudes)* | ✅ Page complete et connectee API | Cloture | M |
| C.6 | `/ia-avancee/recommandations-energie` *(renomme depuis conseiller-energie)* | ✅ Finalisee | Cloture | S |
| C.7 | `/ia-avancee/suggestions-achats` + signaux hub *(remplace prevision-courses)* | ✅ Finalise | Cloture | S |
| C.8 | `/ia-avancee/planning-adaptatif` + `/ia-avancee/adaptations-meteo` *(remplace menu-equilibre)* | ✅ Finalise | Cloture | S |
| C.9 | `/ia-avancee/optimisation-routines` *(renomme depuis routine-optimale)* | ✅ Finalisee | Cloture | S |
| C.10 | `/ia-avancee/prevision-depenses` *(renomme depuis bilan-financier)* | ✅ Finalisee | Cloture | S |
| C.11 | `/ia-avancee/diagnostic-plante` + `/ia-avancee/estimation-travaux` *(remplace assistant-jardin)* | ✅ Finalise | Cloture | S |
| C.12 | `/ia-avancee/suggestions-proactives` *(remplace coach-bien-etre)* | ✅ Finalisee | Cloture | S |
...
|---|-------|-----|----------------|--------|-----------|
| D.1 | Récolte jardin → Recettes saison (proposées au prochain planning semaine) | I1 | Maison/Jardin → Cuisine | M | ☐ Event `jardin.recolte` ☐ Abonné suggestions ☐ Tests |
...
| D.3 | Budget dépassement → Alerte dashboard | I4 | Famille/Budget → Dashboard | S | ☐ Event `budget.depassement` ☐ Widget alerte ☐ Tests |
| D.4 | Inventaire → Courses prédictives | I9 | Inventaire → Courses | L | ☐ ML sur historique ☐ Liste pré-remplie ☐ Tests |
| D.5 | Retour recette → Ajuster planning futur (feedback loop IA) | I13 | Cuisine → Planning | M | ☐ Score recette ☐ Contexte IA ☐ Tests |
| D.6 | Contrats maison → Alertes renouvellement | I14 | Maison/Contrats → Notifications | S | ☐ Job CRON ☐ Multi-canal ☐ Tests |
| D.7 | Enrichir abonnés EventBus (pas juste cache) | — | Global | M | ☐ Refactor bus ☐ Abonnés business ☐ Tests |
...
|---|-------|-----|-------|--------|-----------|
| E.1 | WhatsApp : Flux liste courses semaine | N1 | WhatsApp | M | ☐ Template ☐ Bouton interactif ☐ Tests |
| E.2 | WhatsApp : Rappel activité Jules | N2 | WhatsApp | S | ☐ Template ☐ Suggestion activité ☐ Tests |
| E.3 | WhatsApp : Résultats paris sportifs | N3 | WhatsApp | S | ☐ Template ☐ P&L inline ☐ Tests |
| E.4 | Préférences notifications granulaires — UI par module | N9 | Tous | M | ☐ Schema DB ☐ API ☐ UI settings ☐ Tests |
| E.5 | Centre de notifications dans l'app (historique) | N10 | Tous | M | ☐ Page ☐ Listing ☐ Mark as read ☐ Filtres |
| E.6 | Email : Newsletter hebdo template riche | N5 | Email | M | ☐ Template Jinja2 ☐ Images ☐ Call-to-action |
| E.7 | Email : Rapport budget PDF en pièce jointe | N6 | Email | S | ☐ Attach PDF (déjà généré) |
| E.8 | Push : Actions rapides dans les notifications | N7 | Push | M | ☐ Boutons action ☐ Callback API |

### Nouveaux jobs CRON

| # | Job | Réf | Schedule | Description | Effort |
|---|-----|-----|----------|-------------|--------|
...
| F.3 | Seed data one-click (dev only) | A3 | S | ☐ Endpoint ☐ Données seed ☐ Guard env=dev |
| F.4 | Panneau admin flottant (Ctrl+Shift+A) | — | M | ✅ Overlay React ✅ Trigger jobs ✅ Feature flags ✅ Santé ✅ Quotas IA |
| F.5 | Toggle mode maintenance (bandeau utilisateur) | A10 | S | ✅ Feature flag ✅ Bandeau UI ✅ API toggle |
| F.6 | Queue de notifications — voir et gérer la file d'attente | A8 | S | ✅ API listing ✅ Page admin ✅ Delete/retry |
| F.7 | Historique jobs paginé avec filtres | A9 | M | ☐ API paginée ☐ Filtres status/date ☐ UI |
| F.8 | Reset module (dev only) — vider données d'un module | A4 | S | ✅ Endpoint ✅ Guard env=dev |
...
| # | Tâche | Réf | Flux actuel | Flux cible | Effort |
|---|-------|-----|-------------|------------|--------|
...
| G.5 | Wizard création projet maison (3 étapes) | U5 | Formulaire complet à remplir | **Wizard** : Nom → Type/Budget → Tâches suggérées IA | M |
| G.6 | Alertes proactives inventaire bas → action directe depuis notif | U6 | Cuisine → Inventaire → Parcourir | **Notifications** push quand stock bas | S |
...
| G.8 | Checklist du jour (widget dashboard) avec swipe pour valider | U8 | Maison → Entretien → Voir tâches → Marquer fait | **Widget** : Swipe pour valider dans le dashboard | S |
...
| G.10 | Préférences notifications par module (dans chaque module) | U10 | Paramètres → Section notifications → Toggle par type | **Par module** : "Me notifier pour..." dans chaque module | M |
| G.11 | Mode focus "essentiel du jour" (1 écran) | IN11 | Plusieurs pages à consulter | **1 écran** : météo + repas + tâches + rappels | M |
| G.12 | Mode vacances (pause notifications non essentielles) | IN14 | Pas de fonctionnalité | **Toggle** : Pause + checklist voyage auto | S |
...
| # | Document | Réf | Priorité | Contenu attendu | Effort |
|---|----------|-----|----------|-----------------|--------|
...
| G14 | `/ia-avancee/optimisation-routines` | ✅ Fait | Cloture | C.4 |
| G15 | `/ia-avancee/analyse-photo` | ✅ Fait | Cloture | C.5 |
| G16 | `/ia-avancee/recommandations-energie` | ✅ Fait | Cloture | C.6 |
| G17 | Couvert via `suggestions-achats` et signaux hub | ✅ Fait | Cloture | C.7 |
| G18 | Couvert via `planning-adaptatif` + `adaptations-meteo` | ✅ Fait | Cloture | C.8 |
| G19 | `/ia-avancee/optimisation-routines` | ✅ Fait | Cloture | C.9 |
| G20 | `/ia-avancee/prevision-depenses` | ✅ Fait | Cloture | C.10 |
| G21 | Couvert via `diagnostic-plante` + `estimation-travaux` | ✅ Fait | Cloture | C.11 |
| G22 | `/ia-avancee/suggestions-proactives` | ✅ Fait | Cloture | C.12 |
...
|---|-------------|-------------|--------|----------|--------|
| A1 | **Dashboard admin dédié** | Graphiques : jobs exécutés, erreurs, usage IA, notifications | M | 🟢 | F.1 |
| A2 | **Console IA admin** | Tester un prompt IA directement depuis l'admin → voir réponse brute | S | 🟢 | F.2 |
| A3 | **Seed data one-click** | Bouton pour peupler la DB avec données de test (dev only) | S | 🟡 | F.3 |
| A4 | **Reset module** | Bouton pour vider les données d'un module spécifique (dev only) | S | 🟢 | F.8 |
| A5 | **Export DB complet** | Export JSON/SQL complet de toutes les données utilisateur | M | 🟢 | F.9 |
| A6 | **Import DB** | Import JSON/SQL pour restaurer un backup | M | 🟢 | F.10 |
| A7 | **Monitoring temps réel** | WebSocket admin → métriques live (requêtes/s, erreurs, cache hits) | L | 🟢 | F.11 |
| A8 | **Queue de notifications** | Voir et gérer la file d'attente des notifications pendantes | S | 🟢 | F.6 |
| A9 | **Historique jobs paginated** | Vue paginée de tous les runs de jobs avec filtres | M | 🟡 | F.7 |
