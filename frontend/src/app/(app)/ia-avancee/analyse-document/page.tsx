...
export default function AnalyseDocumentPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Importer un document</CardTitle><CardDescription>Facture, contrat, document administratif ou ordonnance.</CardDescription></CardHeader><CardContent className="space-y-4"><input type="file" title="Choisir un document à analyser" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />{file && <div className="space-y-2 rounded-md border p-3"><p className="text-sm text-muted-foreground">Fichier sélectionné: {file.name}</p>{apercu && <img src={apercu} alt="Aperçu document" className="max-h-64 w-full rounded-md object-cover" loading="lazy" />}</div>}<Button onClick={analyser} disabled={!file || chargement}>{chargement ? 'Analyse...' : 'Analyser le document'}</Button></CardContent></Card>
      {!resultat && !chargement && (
        <Card>
