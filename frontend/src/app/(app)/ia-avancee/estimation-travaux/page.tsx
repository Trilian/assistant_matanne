...
export default function EstimationTravauxPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Décrire le chantier</CardTitle><CardDescription>Ajoute une photo et quelques mots de contexte.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={soumettre}><input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />{file && <div className="space-y-2 rounded-md border p-3"><p className="text-sm text-muted-foreground">Fichier sélectionné: {file.name}</p>{apercu && <img src={apercu} alt="Aperçu travaux" className="max-h-64 w-full rounded-md object-cover" loading="lazy" />}</div>}<Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" /><Button type="submit" disabled={!file || chargement}>{chargement ? 'Analyse...' : 'Estimer les travaux'}</Button></form></CardContent></Card>
      {!resultat && !chargement && (
        <Card>
