...
# =============================================================
# ABONNEMENTS IA  COMPARAISON ET CONSEILS (G6)
# =============================================================


@router.post("/abonnements/ia-comparaison", responses=REPONSES_LISTE)
@gerer_exception_api
async def comparer_abonnements_ia(
    user: dict[str, Any] = Depends(require_auth),
) -> dict[str, Any]:
    """Analyse IA des abonnements actuels et conseils d'optimisation (cache 1h)."""

    def _query() -> dict[str, Any]:
        from src.services.maison.ia.abonnements_ia_service import obtenir_service_abonnements_ia

        with executer_avec_session() as session:
            from src.core.models.abonnements import Abonnement

            abonnements = session.query(Abonnement).all()
            abonnements_data = [
                {
                    "type_abonnement": a.type_abonnement,
                    "fournisseur": a.fournisseur,
                    "prix_mensuel": float(a.prix_mensuel) if a.prix_mensuel else 0.0,
                }
                for a in abonnements
            ]
            total = sum(a["prix_mensuel"] for a in abonnements_data)

        service = obtenir_service_abonnements_ia()
        return service.comparer_abonnements(abonnements_data, total)
