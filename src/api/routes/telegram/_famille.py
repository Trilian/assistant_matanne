...
async def _envoyer_briefing(chat_id: str) -> None:
    """Briefing matinal composite : météo + repas + activités + alertes."""
    from sqlalchemy import func
    ...
    try:
        ...
        actuelle = getattr(meteo, "actuelle", None)
        if actuelle:
            emoji = getattr(actuelle, "emoji", "🌤️")
            condition = html.escape(str(getattr(actuelle, "condition", "variable")))
            temperature = getattr(actuelle, "temperature", "?")
            meteo_ligne = f"{emoji} {condition} — {temperature}°C"
    except Exception as exc:
        logger.warning("Briefing: météo indisponible: %s", exc)

    # --- Récap journée (repas, activités, courses, tâches) ---
    repas_du_jour: list[str] = []
    activites_du_jour: list[str] = []
    courses_restantes = 0
    taches_a_suivre = 0
    ...
    try:
        with obtenir_contexte_db() as session:
            ...
            for item in repas[:4]:
                recette = getattr(item, "recette", None)
                nom = getattr(recette, "nom", None) or getattr(item, "notes", None) or "Repas prévu"
                type_lib = "Midi" if item.type_repas == "dejeuner" else item.type_repas.capitalize()
                repas_du_jour.append(f"{type_lib}: {nom}")
        ...
        logger.warning("Briefing: récap partiel: %s", exc)

    # --- Construction du message ---
    lignes = [f"📋 <b>Briefing du {aujourd_hui.strftime('%A %d/%m')}</b>"]

    if meteo_ligne:
        lignes.append("")
        lignes.append(f"🌦️ {meteo_ligne}")

    if repas_du_jour:
        lignes.append("")
        lignes.append("🍽️ Repas du jour :")
        lignes.extend(f"• {html.escape(str(r))}" for r in repas_du_jour)

    if activites_du_jour:
        lignes.append("")
        lignes.append("🎯 Activités :")
        lignes.extend(f"• {html.escape(str(a))}" for a in activites_du_jour)

    lignes.append("")
    alertes = []
    if courses_restantes:
        alertes.append(f"🛒 {courses_restantes} article(s) en attente")
    if taches_a_suivre:
        alertes.append(f"🧰 {taches_a_suivre} tâche(s) à suivre")
    if alertes:
        lignes.extend(alertes)
    else:
        lignes.append("✅ Rien d'urgent pour le moment !")

    await envoyer_message_interactif(
        destinataire=chat_id,
        corps="\n".join(lignes),
        boutons=[
            {"url": _obtenir_url_app("/ma-journee"), "title": "📋 Ma journée"},
            {"url": _obtenir_url_app("/cuisine/courses"), "title": "🛒 Courses"},
            {"id": "menu_principal", "title": "🏠 Menu principal"},
        ],
    )
