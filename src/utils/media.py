...
class ImageOptimizer:
    ...
    def optimize(self, image: Image.Image, key: str | None = None) -> Image.Image:
        ...
        if image.width > self.config.max_width or image.height > self.config.max_height:
            image.thumbnail(
                (self.config.max_width, self.config.max_height), Image.Resampling.LANCZOS
            )
    ...
    def optimize_bytes(self, image_bytes: bytes, key: str | None = None) -> bytes:
...
# Instance globale
_global_optimizer: ImageOptimizer | None = None
...
def optimize_uploaded_image(
    image_bytes: bytes, config: ImageConfig | None = None, key: str | None = None
) -> bytes:
