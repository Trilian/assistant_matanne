...
class RecipeFeedback(Base):
    ...
    contexte: Mapped[Optional[str]] = mapped_column(String(200))
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "feedback IN ('like', 'dislike', 'neutral')",
...
class OpenFoodFactsCache(Base):
    ...
    nom: Mapped[Optional[str]] = mapped_column(String(300))
    marque: Mapped[Optional[str]] = mapped_column(String(200))
    categorie: Mapped[Optional[str]] = mapped_column(String(200))
    
    # Scores
    nutriscore: Mapped[Optional[str]] = mapped_column(String(1))
    nova_group: Mapped[Optional[int]] = mapped_column(Integer)
    ecoscore: Mapped[Optional[str]] = mapped_column(String(1))
    
    # Données complètes
    nutrition_data: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB)
    allergenes: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB, default=list)
...
class ExternalCalendarConfig(Base):
    ...
    provider: Mapped[str] = mapped_column(
        String(20), 
        nullable=False,
        default=CalendarProvider.GOOGLE.value
    )
    ...
    token_expiry: Mapped[Optional[datetime]] = mapped_column(DateTime)
