...
class ClientIA:
    ...
    # ═══════════════════════════════════════════════════════════
    # VARIANTES RESULT — Retournent Result au lieu de lever
    # ═══════════════════════════════════════════════════════════

    async def appeler_safe(
        self,
        prompt: str,
        prompt_systeme: str = "",
        temperature: float = 0.7,
        ...
        utiliser_cache: bool = True,
        max_tentatives: int = 3,
    ) -> "Result[str, ErreurServiceIA]":
        """Variante de ``appeler()`` qui retourne un ``Result`` au lieu de lever.

        Idéal pour les chaînes fonctionnelles où l'on veut
        traiter l'erreur sans try/except::

            result = await client.appeler_safe("Mon prompt")
            texte = result.unwrap_or("Erreur IA")

            # Ou avec chaînage
            result = (
                await client.appeler_safe("Prompt")
            ).map(lambda r: r.strip()).unwrap_or("")

        Returns:
            Ok(str) en cas de succès, Err(ErreurServiceIA) en cas d'échec.
        """
        from ..result import Err, Ok, Result

        try:
            reponse = await self.appeler(
                prompt=prompt,
                prompt_systeme=prompt_systeme,
                temperature=temperature,
                max_tokens=max_tokens,
            ...
            return Err(
                ErreurServiceIA(
                    str(e),
                    message_utilisateur="Erreur inattendue lors de l'appel IA",
                )
            )

    def generer_json_safe(
        self,
        prompt: str,
        system_prompt: str = "Réponds UNIQUEMENT en JSON valide.",
        temperature: float = 0.3,
        max_tokens: int = 2000,
    ) -> "Result[dict | str, ErreurServiceIA]":
        """Variante de ``generer_json()`` retournant un ``Result``.

        Returns:
            Ok(dict | str) en cas de succès, Err(ErreurServiceIA) en cas d'échec.
        """
        from ..result import Err, Ok, Result

        try:
            reponse = self.generer_json(
                prompt=prompt,
            ...
            if reponse is None:
                return Err(
                    ErreurServiceIA(
                        "Réponse IA vide",
                        message_utilisateur="L'IA n'a pas retourné de réponse",
                    )
                )
            return Ok(reponse)
        except Exception as e:
            return Err(
                ErreurServiceIA(
                    str(e),
                    message_utilisateur="Erreur lors de la génération JSON",
                )
            )
