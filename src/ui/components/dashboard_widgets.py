...
def graphique_inventaire_categories(inventaire: list[dict]) -> go.Figure | None:
    ...
    fig.add_trace(go.Bar(
        y=cat_names,
        x=[t - b for t, b in zip(totaux, bas)],
        name="Stock OK",
        orientation="h",
        marker_color="#4CAF50",
    ))
...
def graphique_activite_semaine(activites: list[dict]) -> go.Figure | None:
    ...
    dates = [a["date"] if isinstance(a["date"], date) else date.fromisoformat(str(a["date"])) for a in activites]
...
def indicateur_sante_systeme() -> dict:
    ...
    try:
        ...
        if verifier_connexion():
            status["details"].append({"nom": "Base de données", "status": "ok", "message": "Connectée"})
        else:
            status["details"].append({"nom": "Base de données", "status": "error", "message": "Déconnectée"})
            status["global"] = "error"
    ...
    try:
        ...
        if hit_rate >= 70:
            status["details"].append({"nom": "Cache", "status": "ok", "message": f"Hit rate: {hit_rate:.0f}%"})
        elif hit_rate >= 40:
            status["details"].append({"nom": "Cache", "status": "warning", "message": f"Hit rate: {hit_rate:.0f}%"})
            if status["global"] == "ok":
                status["global"] = "warning"
        else:
            status["details"].append({"nom": "Cache", "status": "warning", "message": f"Hit rate bas: {hit_rate:.0f}%"})
    except Exception:
