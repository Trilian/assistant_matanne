"""
Mixin d'analyse et calcul pour le planning unifié.

Fournit les méthodes de calcul de charge, détection d'alertes
et statistiques hebdomadaires :
- Score de charge familiale par jour (0-100)
- Détection d'alertes intelligentes (surcharge, activités Jules, budget)
...
class PlanningAnalysisMixin:
    """
    Mixin d'analyse et calcul du planning.

    Fournit les méthodes de calcul de charge, détection d'alertes
    et statistiques. Utilisé par ServicePlanningUnifie.

    Ces méthodes sont pures (pas d'accès DB) et opèrent sur
    les structures de données chargées par PlanningDataLoaderMixin.
    """
