...
class BankrollManager:
    ...
    def calculer_mise_kelly(
        self, bankroll: float, edge: float, cote: float, fraction: float | None = None
    ) -> float:
        """
    ...
    def suggerer_mise(
        self, bankroll: float, edge: float, cote: float, confiance_ia: float
    ) -> SuggestionMise:
        """
    ...
    def calculer_bankroll_actuelle(
        self, user_id: int, bankroll_initiale: float, db: Session
    ) -> float:
        """
        ...
        # Somme des gains
        total_gains = (
            db.query(func.coalesce(func.sum(PariSportif.gain), 0))
        ...
        # Somme des mises
        total_mises = (
            db.query(func.coalesce(func.sum(PariSportif.mise), 0))
    ...
    def obtenir_historique_bankroll(
        self, user_id: int, bankroll_initiale: float, jours: int = 30, db: Session = None
    ) -> list[dict]:
        """
        ...
        paris = (
            ...
            .filter(PariSportif.user_id == user_id, PariSportif.cree_le >= date_debut)
        ...
        # Point initial
        historique.append(
            {"date": date_debut.isoformat(), "bankroll": bankroll_initiale, "variation": 0}
        )
    ...
    def calculer_roi(self, total_mises: float, total_gains: float) -> float:
        """
