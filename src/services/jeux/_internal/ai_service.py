...
"""
        ...
        try:
            ...
            return {
                ...
                "recommandations": reponse.get(
                    "recommandations",
                    ["Continuez à privilégier les value bets", "Respectez votre budget mensuel"],
                ),
                "kpis": kpis,
            }
...
{{
  ...
  "appreciation": "Grille correcte mais déséquilibrée vers les petits numéros."
        ...
        def _normaliser_texte(value: str) -> str:
            ...
            for _ in range(2):
                if "Ã" in sortie or "ã" in sortie or "�" in sortie:
                    try:
                        normalisee = sortie.encode("latin1", errors="ignore").decode(
                            "utf-8", errors="ignore"
                        )
    ...
    def _formater_stats_pour_prompt(self, stats: dict[int, dict], top_n: int = 10) -> str:
        ...
        sorted_stats = sorted(stats.items(), key=lambda x: x[1].get("freq", 0), reverse=True)[
            :top_n
        ]
