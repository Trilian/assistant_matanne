...
from src.core.models.jeux import GrilleEuromillions, GrilleLoto, PariSportif
...
class StatsPersonnellesService:
    ...
    @avec_session_db
    def calculer_roi_global(
        self, user_id: int, jours: int = 30, session: Session | None = None
    ) -> dict[str, Any]:
        """
    ...
    def calculer_win_rate(
        ...
        try:
            ...
            if _has_attr(GrilleLoto, "statut"):
                loto_total_query = loto_total_query.filter(GrilleLoto.statut.in_(_STATUTS_RESOLUS))
            ...
            if _has_attr(GrilleEuromillions, "statut"):
                ...
                euro_gagnants_query = euro_gagnants_query.filter(
                    GrilleEuromillions.gain.isnot(None)
                )
    ...
    @avec_session_db
    def analyser_patterns_gagnants(
        self, user_id: int, jours: int = 90, session: Session | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            strategies_loto = []
            if all(
                _has_attr(GrilleLoto, attr)
                for attr in ("strategie", "backtest", "user_id", "date_creation")
            ):
                loto_statut_filter = (
                    [GrilleLoto.statut.in_(["gagnant", "gagne"])]
                    if _has_attr(GrilleLoto, "statut")
                    else []
                )
                strategies_loto = (
                    session.query(
                        ...
                        func.sum(func.cast(GrilleLoto.backtest["gain"].astext, float)).label(
                            "gains"
                        ),
                    )
                    .filter(
            ...
            if all(
                ...
                euro_statut_filter = (
                    [GrilleEuromillions.statut.in_(["gagnant", "gagne"])]
                    if _has_attr(GrilleEuromillions, "statut")
                    else []
                )
                strategies_euro = (
                    session.query(
                        ...
                        func.sum(
                            func.cast(GrilleEuromillions.backtest["gain"].astext, float)
                        ).label("gains"),
                    )
                    .filter(
                    ...
                    .group_by(GrilleEuromillions.strategie)
                    .all()
                )
            ...
            if meilleure_strat_loto:
                recommandations.append(f"Stratégie Loto la plus rentable: {meilleure_strat_loto}")
    ...
    def obtenir_evolution_mensuelle(
        self, user_id: int, mois: int = 6, session: Session | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            for i in range(mois):
                ...
                if _has_attr(GrilleLoto, "statut"):
                    filtres_loto.append(
                        GrilleLoto.statut.in_(["gagnant", "perdant", "gagne", "perdu"])
                    )
