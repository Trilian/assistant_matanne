...
class EuromillionsIAService(BaseAIService):
    ...
    def _normaliser_stats(self, stats: Any) -> dict[str, Any]:
        ...
        if isinstance(stats, dict):
            ...
            base = {
                ...
                "retards_numeros": getattr(
                    stats, "retards_numeros", getattr(stats, "numeros_retard", {})
                ),
                "retards_etoiles": getattr(
                    stats, "retards_etoiles", getattr(stats, "etoiles_retard", {})
                ),
        ...
        return {
            ...
            or [
                n
                for n, _ in sorted(
                    frequences_numeros.items(), key=lambda item: item[1], reverse=True
                )[:10]
            ],
            ...
            or [
                e
                for e, _ in sorted(
                    frequences_etoiles.items(), key=lambda item: item[1], reverse=True
                )[:5]
            ],
    ...
    @avec_session_db
    def calculer_statistiques(self, jours: int = 365, db: Session | None = None) -> dict[str, Any]:
        """
        ...
        tirages = db.query(TirageEuromillions).filter(TirageEuromillions.date >= date_debut).all()
        ...
        numeros_froids = sorted(freq_numeros.keys(), key=lambda n: freq_numeros[n])[:10]

        etoiles_chaudes = [e for e, _ in freq_etoiles.most_common(5)]

        etoiles_froides = sorted(freq_etoiles.keys(), key=lambda e: freq_etoiles[e])[:5]
    ...
    def generer_grille_equilibree(self, stats: dict[str, Any] | None = None) -> GrilleGeneree:
        """
        ...
        for num in range(self.MIN_NUMERO, self.MAX_NUMERO + 1):
            freq_norm = (
                freq_numeros.get(num, 0) / max(freq_numeros.values()) if freq_numeros else 0.5
            )

            retard_norm = (
                retards_numeros.get(num, 0) / max(retards_numeros.values())
                if retards_numeros
                else 0.5
            )
        ...
        numeros = random.choices(
            population=list(scores.keys()), weights=list(scores.values()), k=self.NB_NUMEROS
        )
        ...
        for etoile in range(self.MIN_ETOILE, self.MAX_ETOILE + 1):
            freq_norm = (
                freq_etoiles.get(etoile, 0) / max(freq_etoiles.values()) if freq_etoiles else 0.5
            )

            retard_norm = (
                retards_etoiles.get(etoile, 0) / max(retards_etoiles.values())
                if retards_etoiles
                else 0.5
            )
    ...
    def generer_grille_frequences(self, stats: dict[str, Any] | None = None) -> GrilleGeneree:
    ...
    def generer_grille_retards(self, stats: dict[str, Any] | None = None) -> GrilleGeneree:
        ...
        numeros_en_retard = sorted(
            retards_numeros.keys(), key=lambda n: retards_numeros[n], reverse=True
        )[:15]

        etoiles_en_retard = sorted(
            retards_etoiles.keys(), key=lambda e: retards_etoiles[e], reverse=True
        )[:6]
    ...
    def generer_grille_ia_creative(self, stats: dict[str, Any] | None = None) -> GrilleGeneree:
        """
...
"""
    ...
    def _analyser_distribution(
        self, numeros: list[int], etoiles: list[int] | None = None
    ) -> dict[str, Any]:
    ...
    def _statistiques_par_defaut(self) -> dict[str, Any]:
        ...
        return {
            "frequences_numeros": dict.fromkeys(range(1, 51), 10),
            "frequences_etoiles": dict.fromkeys(range(1, 13), 5),
            "retards_numeros": dict.fromkeys(range(1, 51), 0),
            "retards_etoiles": dict.fromkeys(range(1, 13), 0),
