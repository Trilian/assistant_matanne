...
class ContexteMaisonService(BaseAIService):
    ...
    def _collecter_jardin(self) -> list[dict]:
        ...
        try:
            ...
            if catalogue_path.exists():
                ...
                plantes = (
                    catalogue.get("plantes", catalogue)
    ...
    def _collecter_cellier_energie(self) -> dict:
        ...
        try:
            ...
            def _check_releves(db=None):
                ...
                dernier = (
                    db.query(ReleveCompteur).order_by(ReleveCompteur.date_releve.desc()).first()
                )
    ...
    def _collecter_contexte_famille(self) -> dict:
        ...
        try:
            ...
            anniversaires = [
                a
                for a in (contexte.get("anniversaires_proches") or [])
            ...
            if any(
                j.get("type") == "creche" and j.get("jours_restants") == 0 for j in jours_speciaux
            ):
            ...
            activites_auj = [
                a for a in (contexte.get("activites_a_venir") or []) if a.get("date") == auj
            ]
...
def obtenir_service_contexte_maison() -> ContexteMaisonService:
    ...
    return obtenir_contexte_maison_service()


# ─── Aliases rétrocompatibilité  ───────────────────────────────

obtenir_contexte_maison_service = obtenir_contexte_maison_service  # alias rétrocompatibilité
