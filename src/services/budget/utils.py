...
def db_entry_to_depense(entry: Any) -> Depense:
    ...
    try:
        frequence = FrequenceRecurrence(entry.frequence_recurrence) if entry.frequence_recurrence else FrequenceRecurrence.PONCTUEL
    except ValueError:
...
def calculer_moyenne_ponderee(valeurs: list[float], poids: list[float] | None = None) -> float:
    ...
    return sum(v * p for v, p in zip(valeurs, poids)) / somme_poids
