...
def _mettre_a_jour_scenario_habitat_apres_projet_termine(event: EvenementDomaine) -> None:
    ...
    try:
        ...
        projet_id = int(event.data.get("projets_id") or event.data.get("projet_id") or 0)
        nom_projet = str(event.data.get("nom") or f"Projet #{projet_id}")
        commentaire = f"Projet terminé pris en compte : {nom_projet}"
        ...
        with obtenir_contexte_db() as session:
            ...
            for scenario in scenarios:
                critere = (
                    session.query(CritereScenarioHabitat)
                    .filter(
                        CritereScenarioHabitat.scenario_id == scenario.id,
                        CritereScenarioHabitat.nom == "valorisation_travaux_maison",
                    )
                    .first()
                )

                if critere is None:
                    critere = CritereScenarioHabitat(
                        scenario_id=scenario.id,
                        nom="valorisation_travaux_maison",
                        poids=Decimal("1.50"),
                        note=Decimal("7.5"),
                    ...
                    try:
                        note_actuelle = Decimal(str(getattr(critere, "note", 0) or 0))
                    except Exception:  # noqa: BLE001
                        note_actuelle = Decimal("0")
                    critere.note = min(Decimal("10.0"), note_actuelle + Decimal("0.5"))
                    texte_existant = str(getattr(critere, "commentaire", "") or "")
                    if nom_projet.lower() not in texte_existant.lower():
                        critere.commentaire = f"{texte_existant} | {commentaire}".strip(" |")

                session.flush()

                avantages = list(getattr(scenario, "avantages", None) or [])
                avantage = f"Valorisation renforcée grâce à {nom_projet}"
                if avantage not in avantages:
                    avantages.append(avantage)
                scenario.avantages = avantages[-5:]

                notes = str(getattr(scenario, "notes", "") or "").strip()
                if nom_projet.lower() not in notes.lower():
                    scenario.notes = f"{notes}\n[auto] {commentaire}".strip()
