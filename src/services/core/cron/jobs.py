...
def _job_alertes_peremption_j3_proactive() -> None:
    ...
    try:
        ...
        lignes_produits = [
            ...
            for a in articles if getattr(a, "date_peremption", None)
        ]
...
def _job_recap_journee() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            nb_repas = int((session.query(func.count(Repas.id)).filter(Repas.date_repas == aujourd_hui).scalar()) or 0)
...
_REGISTRE_JOBS: dict[str, tuple[str, Callable[[], None]]] = {
    ...
    "alertes_peremption_j3_proactive": ("Alertes péremption J-3 proactive", _job_alertes_peremption_j3_proactive),
...
_REGISTRE_JOBS.update(

    {
        ...
        "suggestions_proactives_telegram": ("Suggestions proactives IA Telegram (14h)", _job_suggestions_proactives_telegram),
