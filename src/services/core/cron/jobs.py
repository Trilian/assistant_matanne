...
def _notifier_echec_job_admin(job_id: str, job_name: str, erreur: str) -> None:
    ...
    try:
        ...
        message = f"Le job '{job_name}' ({job_id}) a échoué. Erreur: {erreur[:300]}"
...
def _job_rappels_generaux() -> None:
    ...
    try:
        ...
        try:
            ...
            if nom_recette:
                ingredients_txt = (
                    ", ".join(ingredients[:5]) if ingredients else "vérifie la fiche recette"
                )
...
def _job_push_quotidien() -> None:
    ...
    try:
        from src.services.core.notifications.notif_web_core import obtenir_service_webpush
        from src.services.core.rappels_intelligents import obtenir_rappels_intelligents_service
        ...
        rappels_service = obtenir_rappels_intelligents_service()
        ...
        for rappel in urgents[:5]:  # limiter à 5 push par run
            ...
            nb_envoyes = (
                push_service.envoyer_a_tous(
                    push_service.creer_notification_generique(titre, message)
                )
                if hasattr(push_service, "creer_notification_generique")
                else 0
            )
...
def _job_enrichissement_catalogues() -> None:
    ...
    try:
        ...
        service = obtenir_catalogue_enrichissement_service()
...
def _construire_conseil_cuisine(repas_info: dict[str, object]) -> str | None:
    ...
    if not isinstance(temps_total, int) or temps_total <= 0:
        ...
        if any(mot in nom_lower for mot in ("pâtes", "pates", "omelette", "salade", "wrap")):
            ...
            mot in nom_lower for mot in ("gratin", "lasagne", "rôti", "roti", "curry", "tajine")
        ):
            temps_total = 45
...
def _job_push_contextuel_soir() -> None:
    ...
    try:
        ...
        try:
            with obtenir_contexte_db() as session:
                ...
                for r in repas_demain:
                    ...
                    nom = (
                        recette.nom
                        if recette and getattr(recette, "nom", None)
                        else (r.notes or "Repas")
                    )
        ...
        decongel_msg = f"Pense à décongeler: {', '.join(a_decongeler)}." if a_decongeler else ""
...
def _job_planning_semaine_si_vide() -> None:
    ...
    try:
        ...
        message = f"Aucun planning pour la semaine du {lundi_prochain:%d/%m}. "
...
def _job_alertes_peremption_48h() -> None:
    ...
    try:
        ...
        try:
            ...
            if noms_urgents:
                ...
                suggestions_recettes = [
                    str(row[0]) for row in recettes_avec_urgents if row and row[0]
                ]
        ...
        if articles_critiques_24h:
            ...
            logger.info(
                "J-04: %d produit(s) expirent sous 24h → email critique envoyé",
                len(articles_critiques_24h),
            )
...
def _job_alertes_peremption_j3_proactive() -> None:
    """P4: alerte proactive J-3 avec suggestions de recettes anti-gaspi.

    Complète le job 48h en envoyant un message Telegram ciblé 3 jours avant
    péremption avec des recettes concrètes pour éviter le gaspillage.
    """
    try:
        from datetime import date, timedelta
        ...
        aujourd_hui = date.today()
        seuil_j3 = aujourd_hui + timedelta(days=3)
        seuil_48h = aujourd_hui + timedelta(days=2)
        ...
        with obtenir_contexte_db() as session:
            articles = (
                ...
                .filter(
                    ArticleInventaire.date_peremption.isnot(None),
                    ArticleInventaire.date_peremption > seuil_48h,
                    ArticleInventaire.date_peremption <= seuil_j3,
                    ArticleInventaire.quantite > 0,
                )
                .order_by(ArticleInventaire.date_peremption.asc())
                .limit(8)
        ...
        if not articles:
            logger.info("J-3 proactif: aucun produit entre 48h et 3 jours")
            return
        ...
        if noms_urgents:
            try:
                ...
                suggestions_recettes = [str(row[0]) for row in recettes_trouvees if row and row[0]]
            except Exception:
                logger.debug("J-3: calcul recettes rescue échoué", exc_info=True)

        lignes_produits = [
            f"- {a.nom} (expire le {a.date_peremption:%d/%m})"
            for a in articles
            if getattr(a, "date_peremption", None)
        ]
        message = "🟡 Produits à utiliser sous 3 jours :\n" + "\n".join(lignes_produits)

        if suggestions_recettes:
            message += "\n\n🍽️ Recettes suggérées : " + ", ".join(suggestions_recettes) + "."
        else:
            message += "\n\n💡 Consulte /quoi-manger pour des idées avec ton stock."
        ...
        res = _envoyer_notif_tous_users(
            ...
            message=message,
            canaux=["telegram"],
            titre="Anti-gaspi : produits à utiliser sous 3 jours",
        )
        logger.info("J-3 proactif exécuté: %s", res)
    except Exception:
        logger.exception("Erreur job J-3 proactif")
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            activites = (
                ...
                .filter(
                    ActiviteFamille.date_prevue >= samedi, ActiviteFamille.date_prevue <= dimanche
                )
                .all()
            )
        ...
        message = (
            f"Score weekend: {score}/100. Activités prévues: {nb_activites}. Météo: {meteo_resume}."
        )
...
def _job_rapport_jardin() -> None:
    ...
    try:
        ...
        action_txt = (
            ", ".join(f"{a}:{n}" for a, n in actions[:4]) if actions else "aucune action loggée"
        )
...
def _job_sync_google_calendar() -> None:
    ...
    try:
        ...
        for user_id in user_ids:
            try:
                user_calendriers = service.lister_calendriers_utilisateur(
                ...
                for cal_config in user_calendriers:
                    ...
                    if result.success:
                        ...
                        logger.warning(
                            "J1: sync Google Calendar échouée pour user %s: %s",
                            user_id,
                            result.message,
                        )
...
def _job_alerte_stock_bas() -> None:
    ...
    try:
        ...
        from src.core.db import obtenir_contexte_db
        from src.core.models import ArticleCourses, ArticleInventaire, ListeCourses
        from src.services.core.notifications.notif_dispatcher import get_dispatcher_notifications
...
def _job_rapport_maison_mensuel() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            nb_projets_actifs = session.query(Projet).filter(Projet.statut == "en_cours").count()
    ...
    try:
        ...
        for code in codes_barres:
            try:
                ...
                logger.debug(
                    "J6: impossible de mettre à jour le code-barres %s", code, exc_info=True
                )

        logger.info(
            "J6 sync_openfoodfacts terminée: %d produit(s) mis à jour sur %d",
            nb_maj,
            len(codes_barres),
        )
...
def _job_analyse_nutrition_hebdo() -> None:
    ...
    try:
        ...
        msg_carences = f" Carences possibles: {', '.join(carences)}." if carences else ""
        ...
        _envoyer_notif_tous_users(
            ...
            type_email="resume_hebdo",
            resume={
                "semaine": f"{debut.isoformat()} → {aujourd_hui.isoformat()}",
                "resume_ia": message,
            },
        )
...
def _job_alertes_energie() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            types = [
                r[0]
                for r in session.query(ReleveEnergie.type_energie).distinct().all()
                if r and r[0]
            ]
            ...
            for t in types:
                ...
                historique = [
                    float(r.consommation or 0) for r in releves[1:] if r.consommation is not None
                ]
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            nb_audit = (
                session.execute(
                    text(
                        "DELETE FROM historique_actions WHERE cree_le < (NOW() - INTERVAL '90 days')"
                    )
                ).rowcount
                or 0
            )
            ...
            nb_secu = (
                session.execute(
                    text(
                        "DELETE FROM logs_securite WHERE created_at < (NOW() - INTERVAL '90 days')"
                    )
                ).rowcount
                or 0
            )
...
def _job_resume_jardin_saisonnier() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            nb_elements = (
                session.query(ElementJardin).filter(ElementJardin.statut == "actif").count()
            )
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            payload = {
                ...
                "tables": {
                    "articles_inventaire": int(
                        session.execute(text("SELECT COUNT(*) FROM articles_inventaire")).scalar()
                        or 0
                    ),
                    "listes_courses": int(
                        session.execute(text("SELECT COUNT(*) FROM listes_courses")).scalar() or 0
                    ),
                    "repas": int(session.execute(text("SELECT COUNT(*) FROM repas")).scalar() or 0),
                    "paris_sportifs": int(
                        session.execute(text("SELECT COUNT(*) FROM paris_sportifs")).scalar() or 0
                    ),
                },
            }
...
def _job_anniversaires_j30() -> None:
    ...
    try:
        ...
        for row in rows:
            ...
            if (
                dt
                and getattr(dt, "month", None) == cible.month
                and getattr(dt, "day", None) == cible.day
            ):
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            nb_rep = int(
                session.execute(
                    text(
                        "SELECT COUNT(*) FROM repas WHERE EXTRACT(MONTH FROM date_repas)=:m AND EXTRACT(YEAR FROM date_repas)=:y"
                    ),
...
def _job_recette_du_jour_push() -> None:
    ...
    try:
        ...
        nom = (
            ...
            else (repas.notes or "Repas du jour")
        )
...
def _job_bilan_nutrition_telegram() -> None:
    ...
    try:
        ...
        message = (
            "Bilan nutrition prêt. Ouvre la vue Nutrition pour les détails et recommandations."
        )
...
def _job_stock_critique_zero() -> None:
    ...
    try:
        ...
        res = _envoyer_notif_tous_users(
            ...
            type_email="alerte_critique",
            alerte={
                "titre": "Stock critique — produits épuisés",
                "message": f"Les produits suivants sont à 0 : {noms}. Pensez à les racheter.",
                "lien": "http://localhost:3000/cuisine/inventaire",
            },
        )
...
def _job_resultat_tirage_loto() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            nb_grilles = (
                session.query(GrilleLoto).count() + session.query(GrilleEuromillions).count()
            )
...
def _job_tache_jardin_saisonniere() -> None:
    ...
    try:
        ...
        message = f"Tâches jardin saisonnières: {len(resultats) if isinstance(resultats, list) else 1} suggestion(s)."
...
def _job_stock_prediction_reapprovisionnement() -> None:
    ...
    try:
        ...
        resultats = _envoyer_notif_tous_users(
            dispatcher,
            message=(f"Réapprovisionnement à anticiper cette semaine: {', '.join(alertes[:5])}."),
            canaux=["ntfy", "push"],
...
def _job_variete_repas_alerte() -> None:
    ...
    try:
        ...
        for item in repas:
            ...
            nom = (
                getattr(getattr(item, "recette", None), "nom", None)
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            activites_courantes = (
                session.query(ActiviteFamille)
                .filter(
                    ActiviteFamille.date_prevue >= debut_courant,
            ...
            activites_precedentes = (
                session.query(ActiviteFamille)
                .filter(
                    ActiviteFamille.date_prevue >= debut_prec,
                    ActiviteFamille.date_prevue <= fin_prec,
                )
                .all()
            )
        ...
        terminees = sum(
            1
            for a in activites_courantes
            if str(getattr(a, "statut", "")).lower() in statut_termines
        )
    ...
    try:
        ...
        for type_energie in ("electricite", "gaz", "eau"):
            resultat = service.analyser_anomalies(
                type_energie=type_energie, nb_mois=12, seuil_pct=20
            )
            ...
            for anomalie in anomalies[:2]:
                details.append(
                    f"{type_energie}: {anomalie.get('explication', 'variation détectée')}"
                )
...
def _job_nutrition_adultes_weekly() -> None:
    ...
    try:
        ...
        logger.info(
            "nutrition_adultes exécuté: score=%s notif=%s", bilan.equilibre_score, resultats
        )
...
def _job_jardin_feedback_planning() -> None:
    ...
    try:
        ...
        resultat = get_planning_jardin_service().analyser_recoltes_non_utilisees(
            semaines_lookback=4
        )
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            for abonnement in abonnements:
                ...
                fournisseur_alt = abonnement.fournisseur_alternatif or alternatives.get(
                    type_abo, "offre alternative"
                )
...
def _job_rapport_nutritionnel_jules() -> None:
    ...
    try:
        ...
        resultat = obtenir_service_jules_nutrition_interaction().adapter_planning_nutrition_jules(
            jours_horizon=7
        )
...
def _job_nettoyage_notifications() -> None:
    ...
    try:
        ...
        logger.info(
            "nettoyage_notifications_30j exécuté: %d notification(s) supprimée(s)", nb_supprimees
        )
...
def _job_recap_journee() -> None:
    """Digest de fin de journée multi-modules envoyé sur Telegram/push."""
    ...
    try:
        ...
        message = (
            f"Récap du {aujourd_hui:%d/%m} — "
            f"{nb_repas} repas planifié(s), {nb_activites} activité(s), "
            f"{courses_restantes} article(s) de courses restants, "
            f"{taches_en_retard} tâche(s) à surveiller."
        )
        ...
        _envoyer_notif_tous_users(
            ...
            canaux=["telegram", "push"],
            titre="Récap journée",
            action_url="/ma-journee",
            action_label="Ouvrir Ma journée",
        )

        logger.info("recap_journee exécuté")

    except Exception:
        logger.exception("Erreur job recap_journee")


def _job_suggestion_soiree() -> None:
    """Suggestion légère de soirée couple le vendredi soir."""
    ...
    try:
        ...
        try:
            suggestion = asyncio.run(
                obtenir_service_soiree_ai().suggerer_soirees(
                    budget=70,
                    duree_heures=3.0,
                    type_soiree="detente",
                    region="France",
            ...
            logger.warning("suggestion_soiree: fallback sans IA (%s)", exc)

        message = (
            str(suggestion).strip()
            if suggestion
            else (
                "Suggestion soirée : un dîner tranquille à la maison suivi d'un film ou d'une balade, "
        ...
        _envoyer_notif_tous_users(
            ...
            message=message,
            canaux=["telegram", "push"],
            titre="Suggestion soirée",
            action_url="/famille/weekend",
            action_label="Voir les idées",
        )

        logger.info("suggestion_soiree exécuté")

    except Exception:
        logger.exception("Erreur job suggestion_soiree")
...
_REGISTRE_JOBS: dict[str, tuple[str, Callable[[], None]]] = {
    ...
    "enrichissement_catalogues": (
        "Enrichissement mensuel catalogues IA",
    ...
    "planning_semaine_si_vide": (
        "Vérification planning semaine suivante",
        _job_planning_semaine_si_vide,
    ),
    "alertes_peremption_48h": ("Alertes péremption 48h", _job_alertes_peremption_48h),
    "alertes_peremption_j3_proactive": (
        "Alertes péremption J-3 proactive",
        _job_alertes_peremption_j3_proactive,
    ),
    ...
    "recap_weekend_dimanche_soir": (
        "Recap weekend dimanche soir",
        _job_recap_weekend_dimanche_soir,
    ),
    ...
    "sync_tirages_loto_euromillions": (
        "Sync tirages loto/euromillions",
        _job_sync_tirages_loto_euromillions,
    ),
    ...
    "analyse_tendances_mensuelles": (
        "Analyse tendances mensuelles",
        _job_analyse_tendances_mensuelles,
    ),
    "purge_logs_anciens_mensuelle": (
        "Purge logs anciens mensuelle",
        _job_purge_logs_anciens_mensuelle,
    ),
    ...
    "nettoyage_notifications_30j": (
        "Nettoyage notifications 30 jours",
        _job_nettoyage_notifications,
    ),
    "prediction_depenses": ("Prédiction dépenses fin de mois", _job_prediction_depenses),
    "recap_journee": ("Récapitulatif quotidien multi-modules", _job_recap_journee),
    "suggestion_soiree": ("Suggestion soirée couple", _job_suggestion_soiree),
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            for routine in routines:
                ...
                for tache in taches:
                    heure = _parser_heure(
                        getattr(tache, "heure_prevue", None), routine.moment_journee
                    )
...
def _job_rappel_documents_expirants() -> None:
    ...
    try:
        ...
        urgents = [
            d
            for d in docs_expirants
            if d.jours_avant_expiration is not None and d.jours_avant_expiration < 7
        ]

        proches = [
            d
            for d in docs_expirants
            if d.jours_avant_expiration is not None and 7 <= d.jours_avant_expiration < 30
        ]

        lointains = [
            d
            for d in docs_expirants
            if d.jours_avant_expiration is not None and d.jours_avant_expiration >= 30
        ]
        ...
        if lointains:
            logger.info(
                "rappel_documents: %d document(s) expirent entre 30 et 90 jours", len(lointains)
            )
...
def _job_rapport_mensuel_auto() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            energie_txt = (
                ", ".join(
                    f"{r.type_energie}: {float(r.consommation or 0):.1f} {r.unite}" for r in releves
                )
                if releves
                else "aucun relevé"
            )
            ...
            for t in types:
                ...
                avg_12m = (
                    ...
                    .filter(
                        ...
                        (ReleveEnergie.annee * 12 + ReleveEnergie.mois) >= (annee * 12 + mois - 11),
                        (ReleveEnergie.annee * 12 + ReleveEnergie.mois) <= (annee * 12 + mois),
                    )
                    .scalar()
                )
                ...
                if releve_n1 and releve_n1.consommation:
                    diff_n1 = (
                        (conso - float(releve_n1.consommation)) / float(releve_n1.consommation)
                    ) * 100
...
def _job_rappel_vaccins() -> None:
    ...
    try:
        ...
        if vaccins_en_retard:
            ...
            message = (
                f"RETARD: {len(vaccins_en_retard)} vaccin(s) obligatoire(s) en retard: {noms}."
            )
        ...
        if rappels_proches:
            urgents = [
                v for v in rappels_proches if v.rappel_prevu and v.rappel_prevu <= horizon_7j
            ]
            ...
            if urgents:
                noms = ", ".join(
                    f"{v.nom_vaccin} (J-{(v.rappel_prevu - aujourd_hui).days})" for v in urgents[:5]
                )
            ...
            for voyage in voyages:
                ...
                evenement = EvenementPlanning(
                    ...
                    date_fin=datetime.combine(
                        voyage.date_retour, datetime.max.time().replace(microsecond=0)
                    ),
                    type_event="voyage",
...
def _job_suggestions_saison() -> None:
    ...
    try:
        ...
        if fichier.exists():
            ...
            logger.info(
                "suggestions_saison: %d produit(s) de saison pour %s", len(produits), mois_courant
            )
...
def _job_alertes_budget_seuil() -> None:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            budget_mensuel = (
                session.query(BudgetMensuelDB).filter(BudgetMensuelDB.mois == debut_mois).first()
            )
        ...
        for categorie, budget_prevu in budgets_cat.items():
            ...
            if pourcentage >= 100:
                ...
                alertes.append(
                    f"🟡 {categorie}: {depense_val:.0f}€ / {budget_val:.0f}€ ({pourcentage:.0f}%)"
                )
...
_REGISTRE_JOBS.update(
    {
        ...
        "rapport_jardin_saisonnier": (
            "Rapport jardin saisonnier (mensuel)",
            _job_rapport_jardin_saisonnier,
        ),
        "nettoyage_exports_anciens": (
            "Nettoyage exports > 30 jours",
            _job_nettoyage_exports_anciens,
        ),
        "health_check_services_ia": (
            "Health check services IA (6h)",
            _job_health_check_services_ia,
        ),
        ...
        "s16_resume_weekend_telegram": (
            "Résumé weekend suggestions Telegram",
            _job_resume_weekend_telegram,
        ),
        "s16_rappel_entretien_telegram": (
            "Rappel entretien maison Telegram",
            _job_rappel_entretien_telegram,
        ),
        "s16_bilan_nutrition_telegram": (
            "Bilan nutrition semaine Telegram",
            _job_bilan_nutrition_telegram,
        ),
        "s16_rapport_famille_mensuel": (
            "Rapport famille mensuel complet email/PDF",
            _job_rapport_famille_mensuel_complet,
        ),
        "s16_rapport_maison_trimestriel": (
            "Rapport maison trimestriel email/PDF",
            _job_rapport_maison_trimestriel,
        ),
        "s21_rapport_mensuel_unifie_email": (
            "Rapport mensuel unifié email/PDF",
            _job_rapport_mensuel_unifie_email,
        ),
    }
)
...
def _job_suggestions_proactives_telegram() -> None:
    """Suggestions proactives IA envoyées via Telegram (14h quotidien)."""
    ...
    try:
        ...
        if not result or not result.suggestions:
            logger.debug("Aucune suggestion proactive à envoyer")

            return

        suggestions = result.suggestions[:5]

        lignes = ["💡 *Suggestions du jour*", ""]

        for i, s in enumerate(suggestions, 1):
            emoji = {"haute": "🔴", "normale": "🟠", "basse": "🟢"}.get(
                getattr(s, "priorite", "normale"), "🟠"
            )

            lignes.append(f"{emoji} {i}. **{s.titre}**")

            lignes.append(f"   {s.message}")
        ...
        res = _envoyer_notif_tous_users(
            ...
            message=message,
            canaux=["telegram"],
            titre="Suggestions proactives IA",
        )

        logger.info("Job suggestions proactives Telegram exécuté: %s", res)

    except Exception:
        logger.exception("Erreur job suggestions proactives Telegram")


_REGISTRE_JOBS.update(
    {
        # Nouveaux jobs
        "rappels_jardin_saisonniers": (
            "Rappels jardin saisonniers (lundi 07h)",
            _job_rappels_jardin_saisonniers,
        ),
        "verification_sante_systeme": (
            "Vérification santé système (horaire)",
            _job_verification_sante_systeme,
        ),
        "backup_auto_hebdo_json": ("Backup auto hebdo JSON (dim 04h)", _job_backup_auto_hebdo_json),
        "suggestions_proactives_telegram": (
            "Suggestions proactives IA Telegram (14h)",
            _job_suggestions_proactives_telegram,
        ),
    }
)
...
class DémarreurCron:
    ...
    def _planifier_job(
        self, job_id: str, trigger: CronTrigger, *, replace_existing: bool = False
    ) -> None:
