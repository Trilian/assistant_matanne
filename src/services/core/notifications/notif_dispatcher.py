...
class DispatcherNotifications:
    ...
    def rejouer_dead_letters(self, user_id: str | None = None) -> dict[str, int]:
        ...
        a_rejouer = [
            ...
            if dl.get("statut") == "failed" and (user_id is None or dl.get("user_id") == user_id)
        ]
        ...
        self._dead_letters = [dl for dl in self._dead_letters if dl.get("statut") != "replayed"]
        ...
        **kwargs: Any,
        ...
        for canal in sequence:
            ...
            if strategie == "failover" and resultats.get(canal):
                ...
                logger.debug("Failover canal '%s' échoué, tentative canal suivant…", canal)
    ...
    def _resoudre_canaux(
        ...
        if categorie_cible and categorie_cible in canaux_par_categorie:
            canaux_prefs = [
                c for c in (canaux_par_categorie.get(categorie_cible) or []) if c in _CANAUX_VALIDES
            ]
    ...
    def _envoyer_push(self, user_id: str, message: str, **kwargs: Any) -> bool:
        ...
        try:
            ...
            nb = (
                service.envoyer_a_utilisateur(user_id, notif)
                if hasattr(service, "envoyer_a_utilisateur")
    ...
    def _envoyer_telegram(self, message: str, **kwargs: Any) -> bool:
        ...
        try:
            ...
            from src.services.integrations.telegram import (
                ...
                envoyer_rapport_hebdo_telegram,
            ...
            async def _send() -> bool:
                ...
                if type_message in {"liste_courses", "articles_courses"}:
                    ...
                    return await envoyer_rapport_hebdo_telegram(message)
    ...
    def _envoyer_email(self, user_id: str, message: str, **kwargs: Any) -> bool:
        ...
        if type_email == "alerte_critique":
            ...
            return service_email.envoyer_resume_hebdo(
                email, kwargs.get("resume", {"semaine": "", "resume_ia": message})
            )
            ...
            return service_email.envoyer_rapport_famille_mensuel_complet(
                email, kwargs.get("rapport", {})
            )

        elif type_email == "rapport_maison_trimestriel":
            return service_email.envoyer_rapport_maison_trimestriel(
                email, kwargs.get("rapport", {})
            )
        ...
        elif type_email == "confirmation_backup":
            return service_email.envoyer_confirmation_backup(email, kwargs.get("backup", {}))

        elif type_email == "invitation":
            return service_email.envoyer_invitation_famille(
                email, kwargs.get("invitant", "Matanne")
            )

        else:
            # Générique : alerte critique avec le message

            return service_email.envoyer_alerte_critique(
                email,
                {
