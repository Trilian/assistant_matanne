...
def rappel_jardin_saison():
    ...
    try:
        ...
        catalogue_path = (
            Path(__file__).resolve().parents[3] / "data" / "reference" / "plantes_catalogue.json"
        )
        ...
        conseils_saison = {
            ...
            "hiver": [
                "Protéger du gel",
                "Planifier le jardin du printemps",
...
def tendances_nutrition_hebdo():
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            ...
            logger.info(f"✅ Nutrition semaine: {nb_repas} repas planifiés, {nb_prepares} préparés")
    ...
    try:
        ...
        for user_id in user_ids_connectes:
            try:
                ...
                logger.info(
                    f"✅ Google sync user {user_id}: {imported} importés, {exported} exportés"
                )
...
def resume_mensuel_jeux_telegram():
    ...
    try:
        ...
        logger.info(
            f"✅ Résumé jeux {mois_cible}/{annee_cible} envoyé: {nb_paris} paris, {bilan_net:+.2f}€"
        )
...
def rapport_mensuel_auto() -> None:
    ...
    try:
        ...
        if service_email.envoyer_rapport_mensuel(email_famille, rapport):
            ...
            logger.warning("⚠️ Rapport mensuel %s non envoyé (RESEND_API_KEY manquant ?)", mois_prec)
