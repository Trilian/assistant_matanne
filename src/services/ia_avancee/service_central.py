...
class InnovationsService(BaseAIService):
    ...
    @avec_gestion_erreurs(default_return=None)
    @chronometre("cuisine.idee_repas_soir", seuil_alerte_ms=8000)
    ...
    def _detecter_habitudes(self) -> list[str]:
        ...
        try:
            with obtenir_contexte_db() as session:
                ...
                rows = (
                    session.query(
                        extract("dow", HistoriqueRecette.date_preparation).label("jour"),
                        func.count(HistoriqueRecette.id).label("cnt"),
                    )
                    .group_by(extract("dow", HistoriqueRecette.date_preparation))
