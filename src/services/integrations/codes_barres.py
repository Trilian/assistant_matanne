...
class BarcodeService(BaseService[ArticleInventaire]):
    ...
    def ajouter_article_par_barcode(
        ...
        if existant:
            raise ErreurValidation(f"Ce code-barres est déjÃ  assigné: {existant.nom}")
    ...
    def mettre_a_jour_barcode(
        ...
        logger.info(f"ðŸ”„ Code-barres mis Ã  jour: {ancien_code} â†’ {nouveau_code}")
        self.cache.invalidate()
...
_service_instance: Optional[BarcodeService] = None
