...
class DonneesFacture(BaseModel):
    ...
    date_fin: Optional[date] = Field(default=None, description="Fin période facturée")
    mois_facturation: Optional[int] = Field(default=None, description="Mois principal (1-12)")
    annee_facturation: Optional[int] = Field(default=None, description="Année")
    ...
    # Détails tarif
    prix_kwh: Optional[float] = Field(default=None, description="Prix unitaire kWh")
    abonnement: Optional[float] = Field(default=None, description="Montant abonnement")
...
class ResultatOCR(BaseModel):
    ...
    succes: bool = True
    donnees: Optional[DonneesFacture] = None
    texte_brut: str = ""
    message: str = ""
...
def extraire_montant(texte: str, pattern: str) -> Optional[float]:
