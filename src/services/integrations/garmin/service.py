...
class ServiceGarmin:
    ...
    def get_authorization_url(self, callback_url: str = "oob") -> tuple[str, dict]:
        ...
        if not self.config.consumer_key or not self.config.consumer_secret:
            raise ValueError(
                "Clés Garmin non configurées. Ajoutez GARMIN_CONSUMER_KEY et GARMIN_CONSUMER_SECRET."
            )
...
obtenir_garmin_service = obtenir_garmin_service  # alias rétrocompatibilité 
