...
class PredictionCategorie(BaseModel):
    ...
    tendance: str = Field(..., description="Tendance: hausse, baisse ou stable")
...
class PredictionBudget(BaseModel):
    ...
    confiance: float = Field(..., ge=0, le=1, description="Indice de confiance (0-1)")
...
Sois réaliste et prudent dans tes prédictions. Base-toi sur les tendances observées."""
    ...
    def detecter_anomalies(
        ...
        courant_text = "\n".join(f"- {cat}: {m:.2f}€" for cat, m in depenses_mois_courant.items())
