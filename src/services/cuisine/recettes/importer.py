...
class RecipeImporter:
    ...
    @staticmethod
    def _extract_from_html(soup, url: str) -> dict[str, Any] | None:
        """
        ...
        for script in scripts:
            try:
                ...
                if data.get("@type") == "Recipe" or "recipe" in str(data).lower():
                    ...
                    # Récupérer les temps
                    if "prepTime" in data:
                        recipe["temps_preparation"] = RecipeImporter._parse_duration(
                            data.get("prepTime", "")
                        )
                    if "cookTime" in data:
                        recipe["temps_cuisson"] = RecipeImporter._parse_duration(
                            data.get("cookTime", "")
                        )
            ...
            if ing_container:
                ...
                for ing_div in ing_divs:
                    ...
                    if text and len(text) > 3 and not text.isnumeric():
                        ...
                        if not any(
                            word in text.lower() for word in ["voir", "moins", "filtre", "version"]
                        ):
            ...
            for ol in soup.find_all(["ol", "ul"]):
                ...
                if section_name:
                    ...
                    if any(
                        ...
                        for word in ["étape", "preparation", "instruction", "steps", "directions"]
                    ):
