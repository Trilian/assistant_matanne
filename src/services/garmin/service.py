...
class ServiceGarmin:
    ...
    def complete_authorization(
        ...
        try:
            ...
            # Créer ou mettre Ã  jour le token
    ...
    def sync_user_data(
        ...
        Args:
            ...
            days_back: Nombre de jours Ã  synchroniser en arrière
            db: Session DB (injectée)
        ...
        # Mettre Ã  jour la date de dernière sync
...
def obtenir_service_garmin() -> ServiceGarmin:
    """Factory pour obtenir le service Garmin (convention française)"""
...
def get_or_create_user(username: str, display_name: str, db: Session = None) -> UserProfile:
    ...
    if not user:
        user = UserProfile(
            ...
            avatar_emoji="ðŸ‘©" if username == "anne" else "ðŸ‘¨",
        )
        db.add(user)
        db.commit()
