...
def calculer_statistiques_periode(
    items: list[dict[str, Any]], date_debut: date, date_fin: date
) -> dict[str, int]:
    """
...
def comparer_periodes(
    data_periode1: dict[str, Any], data_periode2: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    def calculer_evolution(val1: float, val2: float) -> dict[str, Any]:
        ...
        return {"evolution": diff, "pourcentage": pct, "tendance": tendance}

    return {
        "recettes": calculer_evolution(
            len(data_periode1.get("recettes", [])), len(data_periode2.get("recettes", []))
        ),
        "courses": calculer_evolution(
            len(data_periode1.get("courses", [])), len(data_periode2.get("courses", []))
        ),
        "activites": calculer_evolution(
            len(data_periode1.get("activites", [])), len(data_periode2.get("activites", []))
        ),
    }
