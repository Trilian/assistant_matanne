...
PrÃeparation des DataFrames pour l'affichage.
"""
...
def _prepare_inventory_dataframe(inventaire: list[dict[str, Any]]) -> pd.DataFrame:
    """PrÃepare un DataFrame pour affichage inventaire"""
    data = []
    for article in inventaire:
        ...
        data.append({
            ...
            "CatÃegorie": article["ingredient_categorie"],
            "QuantitÃe": f"{article['quantite']:.1f} {article['unite']}",
...
def _prepare_alert_dataframe(articles: list[dict[str, Any]]) -> pd.DataFrame:
    """PrÃepare un DataFrame pour affichage alertes"""
    data = []
