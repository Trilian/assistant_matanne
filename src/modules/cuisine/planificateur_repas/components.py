...
from ._common import (
    st, date, timedelta,
    PROTEINES, ROBOTS_CUISINE, TEMPS_CATEGORIES,
...
def render_configuration_preferences():
    """Affiche le formulaire de configuration des prÃefÃerences."""
    
    prefs = charger_preferences()
    
    st.subheader("âš™ï¸ Mes PrÃefÃerences Alimentaires")
    ...
    with st.form("form_preferences"):
        ...
        st.markdown("##### ðŸš« Aliments Ã  Ãeviter")
        exclus = st.text_input(
            "SÃeparÃes par des virgules",
            value=", ".join(prefs.aliments_exclus),
            placeholder="Ex: fruits de mer, abats, coriandre"
        )
        
        st.markdown("##### â¤ï¸ Vos basiques adorÃes")
        ...
        st.markdown("##### âš–ï¸ Équilibre souhaitÃe par semaine")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            poisson = st.number_input("ðŸŸ Poisson", 0, 7, prefs.poisson_par_semaine)
        with col2:
            vege = st.number_input("ðŸ¥¬ VÃegÃetarien", 0, 7, prefs.vegetarien_par_semaine)
        with col3:
        ...
        if st.form_submit_button("ðŸ’¾ Sauvegarder", type="primary"):
            ...
            st.success("âœ… PrÃefÃerences sauvegardÃees!")
            st.rerun()
...
def render_apprentissage_ia():
    ...
    with col1:
        ...
        if likes:
            ...
            st.caption("Pas encore de donnÃees")
...
def render_resume_equilibre(planning_data: dict):
    """Affiche le rÃesumÃe de l'Ãequilibre nutritionnel."""
    
    # Compter les types de protÃeines
    equilibre = {
        "poisson": 0,
        "viande_rouge": 0,
        "volaille": 0,
    ...
    with col2:
        ...
        st.metric("ðŸ¥¬ VÃegÃe", equilibre["vegetarien"], delta=f"{delta:+d}" if delta else None)
    ...
    with col4:
        ...
        st.metric("ðŸ¥e Rouge", equilibre["viande_rouge"], delta=f"{delta:+d}" if delta else None, delta_color=color)
