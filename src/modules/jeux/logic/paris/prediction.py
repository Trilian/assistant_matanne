...
from src.modules.jeux.paris.constants import (
    AVANTAGE_DOMICILE,
...
def predire_resultat_match(
    ...
    return {
        ...
        "niveau_confiance": (
            ...
            if confiance >= SEUIL_CONFIANCE_MOYENNE
            else "faible"
        ),
...
def generer_conseils_avances(
    forme_dom: dict[str, Any], forme_ext: dict[str, Any], cotes: dict[str, float] | None = None
) -> list[dict[str, str]]:
    """
    ...
    buts_moy_dom = (forme_dom.get("buts_marques", 0) + forme_dom.get("buts_encaisses", 0)) / max(
        1, forme_dom.get("nb_matchs", 1)
    )
    buts_moy_ext = (forme_ext.get("buts_marques", 0) + forme_ext.get("buts_encaisses", 0)) / max(
        1, forme_ext.get("nb_matchs", 1)
    )
