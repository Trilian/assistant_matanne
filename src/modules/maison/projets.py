...
def app():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 En cours", "🤖 Assistant IA", "➕ Nouveau", "📄 Tableau"])
    ...
    with tab1:
        ...
        if df_projets.empty:
            ...
            for idx, projet in df_projets.iterrows():
                ...
                with col2:
                    if st.button(
                        "✅ Terminer", key=f"done_{projet['id']}", use_container_width=True
                    ):
                        ...
                        if not taches:
                            ...
                            for t in taches:
                                ...
                                with col_t3:
                                    if t.statut != "termine":
                                        if st.button(
                                            "✅", key=f"task_{t.id}", use_container_width=True
                                        ):
