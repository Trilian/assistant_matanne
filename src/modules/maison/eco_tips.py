...
def get_action_by_id(action_id: int) -> EcoAction | None:
...
def render_stats_dashboard():
    ...
    if stats["cout_initial"] > 0:
        st.info(
            f"💡 Investissement initial: {stats['cout_initial']:.0f}€ | Rentabilise en {stats['roi_mois']:.1f} mois"
        )
...
def render_action_card(action: EcoAction):
    ...
    with st.container(border=True):
        ...
        with col3:
            # Toggle actif
            new_actif = st.checkbox("Actif", value=action.actif, key=f"actif_{action.id}")
...
def render_formulaire(action: EcoAction | None = None):
    ...
    with st.form(f"form_action_{prefix}"):
        ...
        with col1:
            ...
            type_index = (
                types.index(action.type_action) if is_edit and action.type_action in types else 0
            )
            type_action = st.selectbox(
                "Type", options=types, format_func=lambda x: TYPE_LABELS.get(x, x), index=type_index
            )
        ...
        with col2:
            economie_mensuelle = st.number_input(
                "Économie mensuelle (€)",
                min_value=0.0,
                value=float(action.economie_mensuelle)
                if is_edit and action.economie_mensuelle
                else 0.0,
                step=1.0,
            )
            ...
            actif = st.checkbox("Action active", value=action.actif if is_edit else True)
        ...
        if submitted:
            ...
            data = {
                ...
                "economie_mensuelle": Decimal(str(economie_mensuelle))
...
def render_onglet_mes_actions():
    ...
    # Filtrer par type
    filtre = st.radio("Filtrer", options=["Toutes", "Actives", "En pause"], horizontal=True)
