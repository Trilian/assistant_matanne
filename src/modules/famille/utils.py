...
def count_milestones_by_category(child_id: int) -> dict:
    ...
    try:
        with obtenir_contexte_db() as session:
            result = (
                session.query(Milestone.categorie, func.count(Milestone.id).label("count"))
...
def get_budget_mois_dernier() -> float:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            total = (
                session.query(func.sum(FamilyBudget.montant))
...
def get_budget_activites_mois() -> float:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            total = (
                session.query(func.sum(FamilyActivity.cout_reel))
                .filter(
                    and_(
...
def get_stats_sante_semaine() -> dict:
    ...
    try:
        ...
        with obtenir_contexte_db() as session:
            entries = session.query(HealthEntry).filter(HealthEntry.date >= debut_semaine).all()
            ...
            stats = {
                ...
                "energie_moyenne": sum(e.note_energie or 0 for e in entries if e.note_energie)
                / max(len([e for e in entries if e.note_energie]), 1),
                "moral_moyen": sum(e.note_moral or 0 for e in entries if e.note_moral)
                / max(len([e for e in entries if e.note_moral]), 1),
            }
