"""
Logique métier pour le dashboard central
Fonctions utilitaires pour calcul de métriques et notifications
"""
...
def generer_notifications(
    inventaire_critiques: list[dict] | None = None,
    peremption_proche: list[dict] | None = None,
    ...
    if planning_vide:
        notifications.append(
            {"type": "info", "message": "Aucun planning pour cette semaine", "priorite": "basse"}
        )
...
def trier_notifications_par_priorite(notifications: list[dict]) -> list[dict]:
    ...
    return sorted(notifications, key=lambda n: priorites.get(n.get("priorite"), 2))
