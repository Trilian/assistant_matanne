...
def _render_export_tab(service):
    ...
    st.info(
        "Téléchargez vos repas et activités au format iCal pour les importer dans votre application de calendrier préférée."
    )
    ...
    with col2:
        include_activities = st.checkbox(
            "Inclure les activités", value=True, key="export_activities"
        )
