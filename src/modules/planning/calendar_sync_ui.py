...
def render_calendar_sync_ui():
    ...
    # Tabs pour les diffÃerentes options
...
def _render_export_tab(service):
    ...
    st.info("TÃelÃechargez vos repas et activitÃes au format iCal pour les importer dans votre application de calendrier prÃefÃerÃee.")
    
    col1, col2 = st.columns(2)
    with col1:
        include_meals = st.checkbox("Inclure les repas", value=True, key="export_meals")
    with col2:
        include_activities = st.checkbox("Inclure les activitÃes", value=True, key="export_activities")
    
    days_ahead = st.slider("PÃeriode (jours)", 7, 90, 30, key="export_days")
    
    if st.button("ðŸ“¥ GÃenÃerer le fichier iCal", type="primary"):
        ...
        if ical_content:
            st.download_button(
                label="ðŸ’¾ TÃelÃecharger le fichier .ics",
                data=ical_content,
                file_name="assistant_matanne_calendar.ics",
                mime="text/calendar",
            )
            st.success("âœ… Fichier gÃenÃerÃe avec succès!")


def _render_import_tab(service):
    """Onglet d'import iCal."""
    st.markdown("### Importer depuis une URL iCal")
    st.info("Importez des ÃevÃenements depuis un calendrier partagÃe (Google Calendar, iCloud, etc.)")
...
def _render_connect_tab(service):
    ...
    if getattr(params, 'GOOGLE_CLIENT_ID', None):
        ...
        st.warning("Google Calendar non configurÃe (GOOGLE_CLIENT_ID manquant)")
