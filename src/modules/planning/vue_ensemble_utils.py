...
def generer_alertes(
    ...
    if en_retard:
        alertes.append({"type": "danger", "message": f"âš ï¸ {len(en_retard)} tâche(s) en retard"})

    # Tâches urgentes
    urgentes = identifier_taches_urgentes(taches, 3)
    if urgentes:
        alertes.append(
            {"type": "warning", "message": f"⏰ {len(urgentes)} tâche(s) urgente(s) (< 3 jours)"}
        )
    ...
    if evt_aujourdhui:
        alertes.append(
            {"type": "info", "message": f"🎯 {len(evt_aujourdhui)} evenement(s) aujourd'hui"}
        )
