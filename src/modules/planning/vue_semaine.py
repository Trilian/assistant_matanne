...
def afficher_timeline_jour(jour_complet: dict, jour: date) -> None:
    ...
    events_grouped = {
        "🍽️ Repas": jour_complet.get("repas", []),
        "🎨 Activités": jour_complet.get("activites", []),
        "🏗️ Projets": jour_complet.get("projets", []),
...
def app():
    ...
    with col_nav1:
        if st.button("⬅️ Semaine précédente", key="prev_semaine_view"):
    ...
    with tab1:
        ...
        # Couverture Jules
        st.write(
            f"👶 **Activités Jules**: {stats.get('activites_jules', 0)} activités "
    ...
    with tab2:
        ...
        with col_r2:
            ...
            st.metric("🍽️ Repas planifiés", stats.get("total_repas", 0))
