...
def get_jours_semaine(date_ref: date | None = None) -> list[date]:
...
def get_numero_semaine(date_ref: date | None = None) -> int:
...
def filtrer_evenements_semaine(
    evenements: list[dict[str, Any]], date_ref: date | None = None
) -> list[dict[str, Any]]:
...
def grouper_evenements_par_jour(
...
def calculer_statistiques_semaine(
    evenements: list[dict[str, Any]], date_ref: date | None = None
) -> dict[str, Any]:
    ...
    if not evt_semaine:
        return {"total": 0, "par_type": {}, "duree_totale": 0, "duree_moyenne": 0}
...
def suggerer_creneaux_libres(
    evenements: list[dict[str, Any]], jour: date, duree_minutes: int = 60
) -> list[dict[str, Any]]:
    ...
    for evt in evt_tries:
        ...
        if gap_minutes >= duree_minutes:
            creneaux_libres.append(
                {"debut": heure_actuelle, "fin": heure_evt, "duree": int(gap_minutes)}
            )
    ...
    if gap_final >= duree_minutes:
        creneaux_libres.append({"debut": heure_actuelle, "fin": heure_fin, "duree": int(gap_final)})
...
def formater_periode_semaine(date_ref: date | None = None) -> str:
