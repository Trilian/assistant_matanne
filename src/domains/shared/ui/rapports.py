...
def render_rapport_stocks():
    ...
    with col2:
        if st.button("ðŸ‘ï¸ AperÃ§u", key="btn_preview_stocks", use_container_width=True):
            st.session_state.preview_stocks = True
    
    with col3:
        if st.button("ðŸ“¥ TÃ©lÃ©charger PDF", key="btn_download_stocks", use_container_width=True):
    ...
    if st.session_state.get("preview_stocks"):
        try:
            ...
            if donnees.articles_perimes:
                st.subheader("âŒ Articles pÃ©rimÃ©s")
            ...
            if donnees.categories_resumee:
                st.subheader("ðŸ“Š Stock par catÃ©gorie")
                cat_data = []
...
def render_rapport_budget():
    ...
    st.subheader("ðŸ’° Rapport Budget/DÃ©penses")
    ...
    with col2:
        if st.button("ðŸ‘ï¸ AperÃ§u", key="btn_preview_budget", use_container_width=True):
            st.session_state.preview_budget = True
    
    with col3:
        if st.button("ðŸ“¥ TÃ©lÃ©charger PDF", key="btn_download_budget", use_container_width=True):
...
def render_analyse_gaspillage():
    ...
    with col2:
        if st.button("ðŸ‘ï¸ AperÃ§u", key="btn_preview_gaspillage", use_container_width=True):
            st.session_state.preview_gaspillage = True
    
    with col3:
        if st.button("ðŸ“¥ TÃ©lÃ©charger PDF", key="btn_download_gaspillage", use_container_width=True):
    ...
    if st.session_state.get("preview_gaspillage"):
        try:
            ...
            if analyse.categories_gaspillage:
                st.subheader("ðŸ“Š Gaspillage par catÃ©gorie")
            ...
            if analyse.articles_perimes_detail:
                st.subheader("âŒ Articles pÃ©rimÃ©s (dÃ©tail)")
