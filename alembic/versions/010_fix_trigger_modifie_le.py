...
Problème: Le trigger update_updated_at_column() essaie de modifier
NEW.updated_at sur recettes et modeles_courses, mais ces colonnes n'existent pas.

Solution: Ajouter la colonne updated_at à:
- recettes
- modeles_courses

Ces tables utilisaient modifie_le mais les triggers s'attendent à updated_at.
On ajoute la colonne pour que le trigger fonctionne.

Les autres tables (depenses, budgets, config_meteo, etc.) ont déjà updated_at.
"""
...
def upgrade():
    """
    Ajouter colonne updated_at à recettes et modeles_courses
    """
    from alembic import op
    import sqlalchemy as sa
    
    # Ajouter updated_at à recettes
    op.add_column('recettes', sa.Column('updated_at', sa.DateTime(), nullable=True))
    
    # Ajouter updated_at à modeles_courses  
    ...
    # Maintenant qu'il y a des données, on peut le rendre NOT NULL
