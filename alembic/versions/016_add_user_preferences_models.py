...
down_revision: str | None = "014_add_recette_equilibre_fields"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Contraintes temps
        sa.Column(
            "temps_semaine", sa.Integer(), server_default="30", comment="Minutes max en semaine"
        ),
        sa.Column(
            "temps_weekend", sa.Integer(), server_default="90", comment="Minutes max le weekend"
        ),
        # PrÃ©fÃ©rences alimentaires (JSONB arrays)
        sa.Column("aliments_exclus", postgresql.JSONB(astext_type=sa.Text()), server_default="[]"),
        sa.Column(
            "aliments_preferes", postgresql.JSONB(astext_type=sa.Text()), server_default="[]"
        ),
        sa.Column(
            "cuisines_preferees", postgresql.JSONB(astext_type=sa.Text()), server_default="[]"
        ),
        # Ã‰quipements
        sa.Column(
            "robots_disponibles",
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Boolean(),
            nullable=True,
            comment="Si Jules Ã©tait prÃ©sent et a aimÃ©",
        ),
        sa.Column(
            "contexte", sa.String(50), nullable=True, comment="batch_cooking, semaine, weekend"
        ),
        # MÃ©tadonnÃ©es
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column(
            "calendar_id", sa.String(255), nullable=True, comment="ID calendrier spÃ©cifique"
        ),
    ...
    op.create_index(
        "ix_external_calendar_configs_user_id", "external_calendar_configs", ["user_id"]
    )
