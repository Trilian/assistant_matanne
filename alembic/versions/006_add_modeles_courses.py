...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_modeles_courses_utilisateur_id"),
        "modeles_courses",
        ["utilisateur_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["modele_id"],
            ["modeles_courses.id"],
            name=op.f("fk_articles_modeles_modele_id_modeles_courses"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["ingredient_id"],
            ["ingredients.id"],
            name=op.f("fk_articles_modeles_ingredient_id_ingredients"),
            ondelete="SET NULL",
        ),
    ...
    # Create indexes
    op.create_index(
        op.f("ix_articles_modeles_modele_id"), "articles_modeles", ["modele_id"], unique=False
    )
    op.create_index(
        op.f("ix_articles_modeles_ingredient_id"),
        "articles_modeles",
        ["ingredient_id"],
        unique=False,
    )
