...
| SQL consolidation | INIT_COMPLET.sql v3 (~130 tables) | 🟡 V002 incomplète |
...
| `sql/migrations/V002_user_id_standardization.sql` | ⚠️ Partiel | Tables créées, conversions VARCHAR→UUID commentées |
| Alembic | ❌ Archivé intentionnellement | Système custom maintenu |

### Problèmes SQL identifiés

**1. V002 incomplète — user_id hétérogène (impact prod)**

Les tables `preferences_utilisateurs`, `configs_calendriers_externes`, `retours_recettes` utilisent encore `user_id VARCHAR(100)`.
- En dev : `auth.uid()` = `"dev-user"` (string) → ça marche par coïncidence
- En prod Supabase : `auth.uid()` = UUID → RLS bloque tout accès silencieusement

Résolution (sans migration versionnée, après nettoyage données dev) :
```sql
-- Dans INIT_COMPLET.sql directement
UPDATE preferences_utilisateurs SET user_id = uuid_generate_v4()::text WHERE user_id = 'dev-user';
ALTER TABLE preferences_utilisateurs ALTER COLUMN user_id TYPE UUID USING user_id::UUID;
-- (Idem pour retours_recettes et configs_calendriers_externes)
```

**2. Doublon table `liste_courses` / `listes_courses`**
...
**3. Vues potentiellement obsolètes**
...
| Cuisine | Batch preparation → Réduction courses auto (complet) | Décrit mais non terminé |
| Famille | Carnet santé Jules → Planning repas (allergies) | Sécurité alimentaire |
...
| Planning ↔ Jeux | Matchs dans vue semaine | Route planning via jeux.py |
...
|---------|---------------------|-----------------|
| Cuisine ↔ Famille (Jules) | Génération repas Jules depuis planning adultes | Gain de temps majeur |
...
| Famille (Anniversaires) ↔ Courses | J-7 avant anniversaire → ingrédients gâteau en courses | Automation contextuelle |
...
| **Classification photo résidus** | Pixtral → identifier produit périmé → recette urgente | Anti-gaspillage | ⭐⭐ |
...
| `anniversaires_j7` | Quotidien 08:00 | Ingrédients gâteau J-7 avant anniversaire | Famille ↔ Courses |
...
Ce fichier documente Redis mais le projet utilise cash mémoire + fichier. À supprimer/remplacer.
...
| C3 | Fix RGPD user_id vide → HTTPException 401 | `src/api/routes/rgpd.py` | 🟠 Sécurité données |
| C4 | Fix automations GET → extraire mutation vers startup | `src/api/routes/automations.py` | 🟠 Idempotence HTTP |
| C5 | Ajouter `SanitiseurDonnees` dans assistant.py | `src/api/routes/assistant.py` | 🟠 Injection |
| C6 | Fix `Query.get()` → `session.get()` dans auth.py | `src/api/routes/auth.py` | 🟡 Compat SQLAlchemy 2.0 |
| C7 | Remplacer `except Exception: pass` → `logger.warning()` (12 cas) | Tous les concernés | 🟡 Observabilité |
| C8 | Exposer `push_service.obtenir_abonnes()` public | `cron/jobs.py` | 🟡 Encapsulation |
...
| SQL2 | Préparer/décommenter V002 UUID après nettoyage données dev |
| SQL3 | Ajouter 5 index manquants dans INIT_COMPLET.sql |
| SQL4 | Vérifier et supprimer vues obsolètes Streamlit |
| SQL5 | Ajouter triggers `listes_courses.modifie_le` |
