...
class TestInventaireItemBase:
    ...
    def test_inventaire_item_valide(self):
        ...
        item = InventaireItemBase(
            nom="Tomates", quantite=2.5, unite="kg", categorie="Fruits & L̩gumes"
        )
    ...
    def test_inventaire_item_date_peremption(self):
        ...
        item = InventaireItemBase(nom="Yaourt", quantite=1, date_peremption=date_exp)
...
class TestRepasBase:
    ...
    def test_repas_base_valide(self):
        ...
        repas = RepasBase(
            type_repas="dejeuner", date=datetime.now(), recette_id=1, notes="Excellent"
        )
...
class TestCourseItemBase:
    ...
    def test_course_item_valide(self):
        ...
        item = CourseItemBase(nom="Pommes", quantite=2, unite="kg", coche=False)
...
class TestHealthResponse:
    ...
    def test_health_response_creation(self):
        ...
        now = datetime.now()
        response = HealthResponse(status="healthy", version="1.0.0", database="ok", timestamp=now)
...
class TestRateLimitConfig:
    ...
    def test_config_custom(self):
        ...
        config = RateLimitConfig(
            requests_per_minute=100, strategy=RateLimitStrategy.TOKEN_BUCKET, enable_headers=False
        )
...
class TestRateLimiter:
    ...
    def restore_rate_limiter(self):
        ...
        def real_check_rate_limit(
            self, request, user_id=None, is_premium=False, is_ai_endpoint=False
        ):
    ...
    def test_limiter_creation(self):
        ...
        from src.api.rate_limiting import RateLimitConfig, RateLimiter, RateLimitStore
    ...
    def test_limiter_check_anonymous_limits(self):
        ...
        store = RateLimitStore()
        config = RateLimitConfig(
            anonymous_requests_per_minute=50, requests_per_hour=1000, requests_per_day=10000
        )
        limiter = RateLimiter(store=store, config=config)
    ...
    def test_limiter_ai_endpoint_limits(self):
        ...
        store = RateLimitStore()
        config = RateLimitConfig(
            ai_requests_per_minute=30, ai_requests_per_hour=100, ai_requests_per_day=1000
        )
        limiter = RateLimiter(store=store, config=config)
...
class TestRateLimitUtilities:
    ...
    def test_reset_rate_limits(self):
        ...
        from src.api.rate_limiting import _store, reset_rate_limits
...
class TestPaginatedResponse:
    ...
    def test_paginated_response_creation(self):
        ...
        response = PaginatedResponse(
            items=[{"id": 1}, {"id": 2}], total=100, page=1, page_size=20, pages=5
        )
