...
class TestRecetteSuggestion:
    ...
    def test_recette_suggestion_complete(self):
        ...
        suggestion = RecetteSuggestion(
            ...
            type_repas="dÃ©jeuner",
            saison="Ã©tÃ©",
            ingredients=[{"nom": "laitue", "quantite": "1"}, {"nom": "tomates", "quantite": "3"}],
            etapes=[{"description": "Laver les lÃ©gumes"}, {"description": "Couper en morceaux"}],
        )
...
class TestRecetteServiceMethodsMocked:
    ...
    def test_creer_recette(self, mock_client):
        ...
        result = service.creer({"nom": "Nouvelle recette", "temps_preparation": 20})
        assert result.id == 10
...
class TestEdgeCases:
    ...
    def test_version_bebe_age_limite(self):
        ...
        version_min = VersionBebeGeneree(
            instructions_modifiees="Instructions", notes_bebe="Notes", age_minimum_mois=6
        )
        assert version_min.age_minimum_mois == 6

        # Ã‚ge maximum 36 mois
        version_max = VersionBebeGeneree(
            instructions_modifiees="Instructions", notes_bebe="Notes", age_minimum_mois=36
        )
