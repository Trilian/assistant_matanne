...
class TestPredictionServiceMethods:
    ...
    def test_analyser_historique_mixed_types(self, service):
        ...
        assert result is not None
        assert (
            result["nombre_modifications"] == 3
        )  # Seulement les modifications_quantite avec consommation
    ...
    def test_predire_quantite_normal(self, service):
        ...
        result = service.predire_quantite(quantite_actuelle=100, taux_consommation=2.0, jours=30)
        # 100 - (2 * 30) = 100 - 60 = 40
        assert result == 40

    def test_predire_quantite_negative_returns_zero(self, service):
        """PrÃ©diction nÃ©gative retourne 0."""
        result = service.predire_quantite(quantite_actuelle=10, taux_consommation=5.0, jours=30)
        # 10 - (5 * 30) = 10 - 150 = -140 â†’ 0
        assert result == 0

    def test_predire_quantite_zero_consumption(self, service):
        """Consommation nulle garde la quantitÃ©."""
        result = service.predire_quantite(quantite_actuelle=50, taux_consommation=0, jours=30)
        assert result == 50

    def test_predire_quantite_short_period(self, service):
        """PrÃ©diction sur pÃ©riode courte (7 jours)."""
        result = service.predire_quantite(quantite_actuelle=100, taux_consommation=3.0, jours=7)
        # 100 - (3 * 7) = 100 - 21 = 79
        assert result == 79
    ...
    def test_detecter_rupture_taux_zero(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=10, quantite_min=5, taux_consommation=0
        )
    ...
    def test_detecter_rupture_taux_negatif(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=10, quantite_min=5, taux_consommation=-1
        )
        assert risque is False
        assert jours is None
    ...
    def test_detecter_rupture_risque_eleve(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=15, quantite_min=5, taux_consommation=2.0
        )
    ...
    def test_detecter_rupture_limite(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=33, quantite_min=5, taux_consommation=2.0
        )
    ...
    def test_detecter_rupture_sans_risque(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=100, quantite_min=10, taux_consommation=2.0
        )
    ...
    def test_detecter_rupture_deja_en_dessous(self, service):
        ...
        risque, jours = service.detecter_rupture_risque(
            quantite_actuelle=3, quantite_min=5, taux_consommation=1.0
        )
        # (3 - 5) / 1 = -2 â†’ 0
        assert risque is True
        assert jours == 0
...
class TestObteniServicePredictionsSingleton:
    ...
    def test_obtenir_service_predictions_is_prediction_service(self):
        ...
        from src.services.predictions import PredictionService, obtenir_service_predictions
