...
3. Génère un rapport actionnable
...
class TestAnalysis:
    ...
    score: float = 0.0  # 0-100, qualité estimée
...
INEFFECTIVE_PATTERNS = {
    ...
    "pass_body": (r'def test_\w+[^:]+:\s*\n\s+(?:\"\"\"[^\"]*\"\"\"\s*\n\s+)?pass\b', 10, "Tests avec pass"),
...
POSITIVE_PATTERNS = {
    ...
    "real_assert": (r'assert\s+\w+\.\w+\s*==', 2, "Assertion sur attribut réel"),
...
def analyze_test_file(filepath: Path) -> TestAnalysis:
    ...
    if test_density > 3:  # Plus de 3 tests par 50 lignes = probablement hasattr
        score -= 10
        issues.append("Densité tests très haute (tests superficiels?)")
...
def find_source_for_test(test_path: str, source_files: Dict[str, Path]) -> List[str]:
    """Trouve les fichiers source correspondant à un test."""
...
def run_audit(project_root: Path, output_csv: Path):
    ...
    print(f"    {len(test_analyses)} fichiers de test analysés")
    ...
    print(f"\n[*] Génération du rapport: {output_csv}")
    output_csv.parent.mkdir(parents=True, exist_ok=True)
    
    with open(output_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([
            "type", "path", "lines", "nb_tests", "quality_score",
            "mapped_sources", "issues", "action"
        ])
    ...
    print(f"\n[+] Tests de qualité (>=70): {len(good_tests)}")
    print(f"[~] Tests à améliorer (50-70): {len(low_quality_tests)}")
    print(f"[-] Tests inefficaces (<50): {len(ineffective_tests)}")
    print(f"[!] Sources sans tests: {len(uncovered_sources)}")
    
    # Top tests à supprimer/réécrire
