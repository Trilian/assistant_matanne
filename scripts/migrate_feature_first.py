    ...
    if ui_path.exists():
        ...
        for item in ui_path.iterdir():
            if item.is_dir() and item.name != "__pycache__":
                ...
                if not new_path.exists():
                    ...
                    MOVED_FILES[f"src/modules/{module_name}/ui/{item.name}"] = f"src/modules/{module_name}/{item.name}"
                else:
        ...
        for file in ui_path.glob("*.py"):
            if file.name != "__init__.py":
                ...
                MOVED_FILES[f"src/modules/{module_name}/ui/{file.name}"] = f"src/modules/{module_name}/{file.name}"
            ...
            if file.name.endswith("_logic.py"):
                ...
                if feature_dir.exists() and feature_dir.is_dir():
                    ...
                    if not dest.exists():
                        ...
                        MOVED_FILES[f"src/modules/{module_name}/logic/{file.name}"] = f"src/modules/{module_name}/{feature_name}/utils.py"
                    else:
                    ...
                    git_mv(str(file), str(dest))
                    MOVED_FILES[f"src/modules/{module_name}/logic/{file.name}"] = f"src/modules/{module_name}/{feature_name}_utils.py"
            
            elif file.name == "schemas.py":
                # Special case: schemas stays at module root
                git_mv(str(file), str(module_path / "schemas.py"))
                MOVED_FILES[f"src/modules/{module_name}/logic/schemas.py"] = f"src/modules/{module_name}/schemas.py"
            
            else:
                # Other logic files move to module root
                git_mv(str(file), str(module_path / file.name))
                MOVED_FILES[f"src/modules/{module_name}/logic/{file.name}"] = f"src/modules/{module_name}/{file.name}"
