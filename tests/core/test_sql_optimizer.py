...
Couvre :
- SQLAlchemyListener (log, extract, clear)
- N1Detector (analyze, suggestions)
- BatchLoader
...
def test_log_query_adds_query(mock_session_state):
    SQLAlchemyListener._log_query("SELECT * FROM test", 120, {})
    assert SQLAlchemyListener.SESSION_KEY in mock_session_state
    assert len(mock_session_state[SQLAlchemyListener.SESSION_KEY]) == 1
    q = mock_session_state[SQLAlchemyListener.SESSION_KEY][0]
    assert q.operation == "SELECT"
    assert q.table == "test"
    assert q.duration_ms == 120

def test_extract_operation_and_table():
    op = SQLAlchemyListener._extract_operation("SELECT * FROM foo")
    table = SQLAlchemyListener._extract_table("SELECT * FROM foo")
    assert op == "SELECT"
    assert table == "foo"

def test_get_stats_returns_dict(mock_session_state):
    SQLAlchemyListener._log_query("SELECT * FROM foo", 50, {})
...
def test_clear_resets_log(mock_session_state):
    ...
    assert mock_session_state[SQLAlchemyListener.SESSION_KEY] == []

def test_n1_detector_analyze_detects_n1(mock_session_state):
    # Inject 6 similar SELECT queries
    for _ in range(6):
        SQLAlchemyListener._log_query("SELECT * FROM foo WHERE bar_id = 1", 10, {})
