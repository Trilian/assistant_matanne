...
- P8-03: sync_contrats_alertes (hebdo)
- P8-04: check_garanties_expirant (hebdo)
...
class TestPhase8JobsSchedules:
    ...
    PHASE8_JOBS = [
        ...
        "sync_contrats_alertes",
        "check_garanties_expirant",
        "bilan_energetique",
        "rappel_vaccins",
...
# ═══════════════════════════════════════════════════════════
# P8-03 — Sync contrats alertes
# ═══════════════════════════════════════════════════════════


class TestSyncContratsAlertes:

    def test_contrats_a_renouveler(self):
        """Contrats avec renouvellement proche → notification envoyée."""
        from src.services.core.cron.jobs import _job_sync_contrats_alertes

        dans_30j = date.today() + timedelta(days=30)
        contrat = SimpleNamespace(
            nom="Assurance habitation",
            fournisseur="AXA",
            statut="actif",
            date_renouvellement=dans_30j,
            date_fin=None,
            tacite_reconduction=True,
            date_limite_resiliation=None,
        )

        mock_session = MagicMock()
        # auto-expire: aucun contrat à expirer
        mock_session.query.return_value.filter.return_value.all.side_effect = [
            [],  # contrats_a_expirer
            [contrat],  # contrats_renouvellement
        ...
        with (
            ...
            _job_sync_contrats_alertes()

        mock_dispatcher.envoyer.assert_called()
        call_kwargs = mock_dispatcher.envoyer.call_args[1]
        assert "Assurance habitation" in call_kwargs.get("message", "")

    def test_aucun_contrat_pas_de_notification(self):
        """Sans contrat à alerter, le dispatcher ne doit pas être appelé."""
        from src.services.core.cron.jobs import _job_sync_contrats_alertes
...
# ═══════════════════════════════════════════════════════════
# P8-04 — Check garanties expirant
# ═══════════════════════════════════════════════════════════


class TestCheckGarantiesExpirant:

    def test_garanties_critiques_email(self):
        """Garanties expirant sous 14 jours → email critique envoyé."""
        from src.services.core.cron.jobs import _job_check_garanties_expirant

        dans_10j = date.today() + timedelta(days=10)
        garantie = SimpleNamespace(
            appareil="Lave-vaisselle",
            statut="active",
            date_fin_garantie=dans_10j,
        )

        mock_session = MagicMock()
        mock_session.query.return_value.filter.return_value.order_by.return_value.all.return_value = [garantie]
        ...
        with (
            ...
            _job_check_garanties_expirant()

        mock_dispatcher.envoyer.assert_called()
        call_kwargs = mock_dispatcher.envoyer.call_args[1]
        assert "email" in call_kwargs.get("canaux", [])
        assert "Lave-vaisselle" in call_kwargs.get("message", "")

    def test_aucune_garantie(self):
        """Sans garantie expirant, aucune notification."""
        from src.services.core.cron.jobs import _job_check_garanties_expirant
