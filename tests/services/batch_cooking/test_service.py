...
Ce fichier vise Ã  augmenter la couverture des méthodes non testées:
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TESTS - OBTENIR SESSION (AVEC RÃ‰SULTAT)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TESTS - OBTENIR SESSIONS PLANIFIÃ‰ES (AVEC FILTRES)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class TestSuggererRecettesBatch:
    ...
    @patch('src.services.batch_cooking.service.obtenir_client_ia')
    def test_suggerer_recettes_batch_with_robots_monsieur_cuisine(self, mock_client, patched_db_context):
        ...
        result = service.suggerer_recettes_batch(
            nb_recettes=3,
            robots_disponibles=["monsieur_cuisine"],
            db=patched_db_context
        )
...
class TestAttribuerPreparationsPlanning:
    ...
    @patch('src.services.batch_cooking.service.obtenir_client_ia')
    def test_attribuer_preparations_planning_session_not_found(self, mock_client, patched_db_context):
    ...
    def test_attribuer_preparations_planning_with_planning(self, mock_client, patched_db_context):
        ...
        # 4. Ajouter une préparation Ã  la session
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TESTS - GENERER PLAN IA (BRANCHES SUPPLÃ‰MENTAIRES)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class TestFactoryFunctions:
    ...
    def test_alias_batch_cooking_service(self, mock_client):
        ...
        assert BatchCookingService is ServiceBatchCooking


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TESTS - CRÃ‰ATION PRÃ‰PARATION COMPLÃˆTE
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TESTS - COUVERTURE COMPLÃˆTE UPDATE_CONFIG
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
class TestAjouterEtapes:
    ...
    def test_ajouter_etapes_success(self, mock_client, patched_db_context):
        """ajouter_etapes ajoute les étapes Ã  la session."""
        mock_client.return_value = Mock()
        ...
        etapes = [
            ...
            },
            {
                ...
                "description": "Enfourner Ã  180Â°C",
                "duree_minutes": 45,
                "recette_id": 1,
                "groupe_parallele": 1,
                "robots": ["four"],
...
class TestTerminerEtape:
    ...
    def test_terminer_etape_success(self, mock_client, patched_db_context):
        ...
        result = service.ajouter_etapes(
            session_id=session.id,
            etapes=[{"titre": "Test étape termine"}],
            db=patched_db_context
        )
...
class TestPasserEtape:
    ...
    def test_passer_etape_success(self, mock_client, patched_db_context):
        ...
        result = service.ajouter_etapes(
            session_id=session.id,
            etapes=[{"titre": "Test étape passer"}],
            db=patched_db_context
        )
...
class TestGetPreparationsAlertes:
    ...
    def test_get_preparations_alertes_with_alerts(self, mock_client, patched_db_context):
        """get_preparations_alertes retourne les préparations Ã  risque."""
        mock_client.return_value = Mock()
