...
from src.core.models import ArticleInventaire, Ingredient, HistoriqueInventaire
...
class TestEnregistrerModification:
    ...
    def test_enregistrer_modification_succes(self, service_with_history, sample_article, db: Session):
        ...
        # Vérifier en base
        historique = db.query(HistoriqueInventaire).filter_by(
            article_id=sample_article.id
        ).first()
    ...
    def test_enregistrer_modification_emplacement(self, service_with_history, sample_article, db: Session):
...
class TestGetStatistiques:
    ...
    def test_get_statistiques_vide(self, service):
        ...
        service.get_inventaire_complet = MagicMock(return_value=[])
        service.get_alertes = MagicMock(return_value={"stock_bas": [], "critique": [], "peremption_proche": []})
    ...
    def test_get_statistiques_avec_articles(self, service):
        ...
        mock_inventaire = [
            {"id": 1, "quantite": 5.0, "emplacement": "Frigo", "ingredient_categorie": "Légumes", "statut": "ok", "derniere_maj": datetime.utcnow()},
            {"id": 2, "quantite": 2.0, "emplacement": "Placard", "ingredient_categorie": "Féculents", "statut": "stock_bas", "derniere_maj": datetime.utcnow()},
        ]
        service.get_inventaire_complet = MagicMock(return_value=mock_inventaire)
        service.get_alertes = MagicMock(return_value={"stock_bas": [mock_inventaire[1]], "critique": [], "peremption_proche": []})
...
class TestGetStatsParCategorie:
    ...
    def test_get_stats_par_categorie(self, service):
        ...
        mock_inventaire = [
            {"id": 1, "quantite": 5.0, "quantite_min": 2.0, "ingredient_categorie": "Légumes", "statut": "ok"},
            {"id": 2, "quantite": 0.5, "quantite_min": 2.0, "ingredient_categorie": "Légumes", "statut": "critique"},
        ]
        service.get_inventaire_complet = MagicMock(return_value=mock_inventaire)
