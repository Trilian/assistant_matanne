...
# -----------------------------------------------------------------------------
# B-02 - Endpoint cle publique VAPID
# -----------------------------------------------------------------------------
...
class TestVapidPublicKey:
    ...
    async def test_retourne_public_key(self, async_client: httpx.AsyncClient):
        """La reponse doit contenir le champ public_key."""
        response = await async_client.get("/api/v1/push/vapid-public-key")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_public_key_est_une_chaine(self, async_client: httpx.AsyncClient):
        """La cle publique doit etre une chaine non vide."""
        response = await async_client.get("/api/v1/push/vapid-public-key")
        data = response.json()
    ...
    async def test_endpoint_sans_auth(self):
        """L'endpoint VAPID ne necessite pas d'authentification."""
        from src.api.main import app
...
class TestSchedulerJobs:
    """Tests de la presence des jobs planifies dans DemarreurCron."""

    def _get_job_ids(self) -> list[str]:
        """Instancie DemarreurCron et retourne les IDs des jobs configures."""
    ...
    def test_digest_ntfy_dans_scheduler(self):
        """Le job digest_ntfy doit etre enregistre dans le scheduler."""
        job_ids = self._get_job_ids()
        assert "digest_ntfy" in job_ids, (
            f"Job 'digest_ntfy' absent du scheduler. Jobs presents : {job_ids}"
        )

    def test_rappel_courses_dans_scheduler(self):
        """Le job rappel_courses doit etre enregistre dans le scheduler."""
        job_ids = self._get_job_ids()
        assert "rappel_courses" in job_ids, (
            f"Job 'rappel_courses' absent du scheduler. Jobs presents : {job_ids}"
        )

    def test_push_quotidien_dans_scheduler(self):
        """Le job push_quotidien doit etre enregistre dans le scheduler."""
        job_ids = self._get_job_ids()
        assert "push_quotidien" in job_ids

    def test_scheduler_a_au_moins_5_jobs(self):
        """Le scheduler doit avoir au moins 5 jobs configures."""
        job_ids = self._get_job_ids()
        assert len(job_ids) >= 5, f"Seulement {len(job_ids)} jobs configures : {job_ids}"
...
class TestSubscribePersistance:
    ...
    async def test_subscribe_appelle_sauvegarder_abonnement(
        ...
        """sauvegarder_abonnement doit etre appele lors d'un POST /subscribe."""
    ...
    async def test_subscribe_passe_user_id_correct(
        ...
        """sauvegarder_abonnement doit recevoir le user_id de l'utilisateur authentifie."""
        mock_service = MagicMock()
        mock_service.sauvegarder_abonnement.return_value = MagicMock(
            endpoint=subscription_payload["endpoint"],
            user_id="test-user",
    ...
    async def test_subscribe_reponse_contient_endpoint(
        ...
        """La reponse doit contenir l'endpoint de l'abonnement."""
        mock_service = MagicMock()
        mock_service.sauvegarder_abonnement.return_value = MagicMock(
            endpoint=subscription_payload["endpoint"],
            user_id="test-user",
...
class TestUnsubscribe:
    ...
    async def test_unsubscribe_appelle_supprimer_abonnement(
        ...
        """supprimer_abonnement doit etre appele lors du desabonnement."""
        mock_service = MagicMock()
        endpoint = "https://fcm.googleapis.com/fcm/send/test-token-123"
    ...
    async def test_unsubscribe_reponse_success(self, async_client: httpx.AsyncClient):
        """La reponse du desabonnement doit indiquer success=True."""
        mock_service = MagicMock()
