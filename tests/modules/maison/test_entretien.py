...
class TestEntretienService:
    ...
    @patch("src.modules.maison.entretien.ClientIA")
    ...
    async def test_creer_routine(self, mock_client_class):
        ...
        result = await service.creer_routine("Ménage matin", "routine quotidienne")
    ...
    async def test_optimiser_semaine(self, mock_client_class):
        ...
        service = EntretienService()
        service.call_with_cache = AsyncMock(return_value="Lundi: ...\nMardi: ...")
    ...
    @pytest.mark.asyncio
    @patch("src.modules.maison.entretien.ClientIA")
    async def test_conseil_temps_estime(self, mock_client_class):
        """Test estimation temps tâche."""
        mock_client_class.return_value = MagicMock()

        from src.modules.maison.entretien import EntretienService

        service = EntretienService()
        service.call_with_cache = AsyncMock(return_value="30-45 min")

        result = await service.conseil_temps_estime("Nettoyage cuisine")

        assert result == "30-45 min"
...
class TestApp:
    ...
    @patch("src.modules.maison.entretien.st")
    @patch("src.modules.maison.entretien.get_entretien_service")
    @patch("src.modules.maison.entretien.get_stats_entretien")
    def test_app_import(self, mock_stats, mock_service, mock_st):
    ...
    def test_app_runs(self, mock_taches, mock_routines, mock_stats, mock_service, mock_st):
