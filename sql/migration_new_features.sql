...
CREATE TABLE IF NOT EXISTS depenses (
    ...
    tags JSONB DEFAULT '[]',
    famille_id BIGINT REFERENCES familles(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_depenses_famille ON depenses(famille_id);
...
COMMENT ON TABLE config_meteo IS 'Configuration météo par famille';
...
CREATE OR REPLACE FUNCTION get_depenses_mois(p_famille_id BIGINT, p_mois DATE)
RETURNS TABLE(
    categorie VARCHAR,
    total DECIMAL,
    count BIGINT
...
CREATE OR REPLACE FUNCTION get_resume_budget(p_famille_id BIGINT, p_mois DATE)
RETURNS JSONB AS $$
DECLARE
...
DROP POLICY IF EXISTS depenses_famille_policy ON depenses;
CREATE POLICY depenses_famille_policy ON depenses
    FOR ALL
    USING (
        famille_id IN (
            SELECT famille_id FROM membres_famille 
            WHERE user_id = auth.uid()
        )
    );

-- Policies pour budgets_mensuels
DROP POLICY IF EXISTS budgets_famille_policy ON budgets_mensuels;
CREATE POLICY budgets_famille_policy ON budgets_mensuels
    FOR ALL
    USING (
        famille_id IN (
            SELECT famille_id FROM membres_famille 
