...
- **Routes IA stabilisées par domaine** : l'ancien shim `src/api/routes/innovations.py` a été retiré ; les alias historiques ont été absorbés par `recettes.py`, `planning.py`, `famille.py`, `dashboard.py`, `rapports.py`, `courses.py`, `habitat.py`, `jeux.py`, `preferences.py` et `batch_cooking.py`.
