...
def afficher_menu():
    ...
    print("\n╔════════════════════════════════════════════════════════════╗")
    print("║         🗄️ Migration Supabase - Code-Barres/Rapports      ║")
    print("╚════════════════════════════════════════════════════════════╝")
    print("\n📋 Options:")
    print("  1. Vérifier l'état actuel")
...
def verifier_connexion():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            print(f"✅ Connexion à Supabase: OK")
...
def verifier_colonnes(engine):
    ...
    print("\n📊 État actuel de la table ArticleInventaire:")
    try:
        with engine.connect() as conn:
            # Vérifier colonnes
            query = """
            ...
            for col_name, col_type, nullable in result:
                ...
                print(f"     • {col_name:25} {col_type:15} {nullable_str}")
                colonnes[col_name] = col_type
            
            # Vérifier colonnes nouvelles
            print("\n   Statut nouvelles colonnes:")
            if 'code_barres' in colonnes:
                print(f"     ✅ code_barres existe ({colonnes['code_barres']})")
            else:
                print(f"     ❌ code_barres MANQUANTE")
            
            if 'prix_unitaire' in colonnes:
                print(f"     ✅ prix_unitaire existe ({colonnes['prix_unitaire']})")
            else:
                print(f"     ❌ prix_unitaire MANQUANTE")
...
def main():
    ...
    while True:
        ...
        if choix == "1":
            ...
            print("\n⚠️  Downgrade non implémenté pour ce script")
